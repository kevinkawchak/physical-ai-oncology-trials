...
class SurveillanceOrchestrator:
    ...
    def __init__(self, patient_state: PatientJourneyState) -> None:
        ...
        logger.info(
            "SurveillanceOrchestrator initialised: %d cycles on record, entering surveillance per 21 CFR 312.85",
    ...
    def record_treatment_completion(self) -> dict:
        ...
        """
        logger.info(
            "Recording treatment completion: CR, %d/%d cycles per 21 CFR 50.32 and ICH E6(R3) section 2.9",
            PLANNED_CYCLES,
            PLANNED_CYCLES,
        )
    ...
    def transition_to_surveillance_twin(self) -> dict:
        ...
        """
        logger.info("Transitioning digital twin to recurrence detection mode per ICH E6(R3) section 1.4.2")
    ...
    def process_surveillance_imaging(
        ...
        """
        logger.info(
            "Processing %d surveillance imaging assessments per 21 CFR 312.87",
            len(imaging_results),
        )
        ...
        for img in imaging_results:
            ...
            tp = ImagingTimepoint(
                ...
                notes=(f"Month {month} surveillance: NED, recurrence risk {risk_pct}% per 21 CFR 312.87"),
            )
            timepoints.append(tp)
            self._imaging_results.append(tp)

            logger.info(
                "Month %d imaging: NED, recurrence risk %.1f%% per 21 CFR 312.87",
                month,
                risk_pct,
            )
    ...
    def monitor_long_term_safety(self) -> dict:
        ...
        """
        logger.info(
            "Monitoring long-term safety per 21 CFR 312.88 — %d categories assessed",
    ...
    def collect_follow_up_data(
        ...
        """
        logger.info(
            "Collecting follow-up data for %d visits per ICH E6(R3) section 2.9 and 21 CFR 50.32",
            len(visit_data),
        )
        ...
        for visit in visit_data:
            ...
            pro_result = {
                ...
                "EORTC_QLQ_LC13": {
                    ...
                    "peripheral_neuropathy": pro_raw.get("qlq_lc13_neuropathy", 8.0),
                },
            }
            ...
            processed = {
                ...
                "day": month * 30,
                "clinical_assessment": {
                    "recurrence_status": clinical.get("recurrence_status", "NED"),
        ...
        avg_ghs = sum(v["pro_scores"]["EORTC_QLQ_C30"]["global_health_status"] for v in visits_processed) / max(
            len(visits_processed), 1
        )
    ...
    def run(
        ...
        """
        logger.info("Starting Stage 9: Treatment Completion & Surveillance (Month 24 to Month 36) per 21 CFR 312.85")
        state = patient_state
        ...
        state.add_regulatory_event(
            RegulatoryEvent(
                ...
                day=state.get_current_day(),
                description=("Digital twin transitioned to recurrence detection mode per ICH E6(R3) section 1.4.2"),
        ...
        logger.info(
            "Stage 9 complete: CR, %d/%d cycles, event-free, recurrence risk %.1f%%, PFS %dd, OS %dd",
            PLANNED_CYCLES,
            PLANNED_CYCLES,
