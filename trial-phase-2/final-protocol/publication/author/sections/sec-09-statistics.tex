...
Analysis Plan that this section summarizes, and are reported in accordance with
CONSORT statement for parallel-group randomized trials \cite{Schulz2010consort} and TRIPOD+AI statement for the model-assisted advisory component
...
interval. The test is two-sided at $\alpha = 0.05$ level, with type I error
allocated across one interim and final analysis by group-sequential alpha
...
enrolled, allowing for up to $10\%$ non-evaluability while retaining $220$ evaluable RCT participants.
...
\node[mmstep](itt)  at (4.4,2.1)    {ITT population\\all randomized\\(primary)};
...
\node[mmstep](saf)  at (4.4,-4.2)   {Safety population\\intervention\\as received};
\node[mmdec] (intm) at (10.3,2.1)   {Interim at $\sim 60\%$\\events ($\sim 84$):\\DSMB review};
\node[mmgoal](bound)at (8.3,-0.6)     {O'Brien-Fleming\\efficacy / futility\\boundary crossed};
\node[mmgoal](cont) at (12,-0.4){Continue to\\final analysis};
\draw[mmedge] (rand.north east) -- (itt.south west);
\draw[mmedge] (rand.east) -- (mitt.west);
\draw[mmedge] (rand.east) -- (pp.west);
\draw[mmedge] (rand.south east) -- (saf.north west);
\draw[mmedge] (itt) -- (intm);
\draw[mmedge] (intm.south west) -- node[left,font=\scriptsize, yshift=0cm, xshift=-0.1cm]{crossed} (bound.north);
\draw[mmedge] (intm.south east) -- node[left,font=\scriptsize,pos=0.6, yshift=0.15cm, xshift=-0.1cm]{within} (cont.north);
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 18. The four analysis populations (intention-to-treat,
modified intention-to-treat, per-protocol, safety) and single group-sequential
interim analysis at approximately $60\%$ of targeted progression-free-survival
...
device-related SAE rate breach or a hard-limit breach triggers a DSMB review.}
