...
\node[mmstep](map)  at (4.8,0)    {Sensor to x,y,z\\Cartesian command\\frame};
\node[mmgoal](llm)  at (9.5,0)    {On-premises LLM\\advisory, hash-pinned\\to commit + seed};
\node[mmdark] (gate) at (9.5,-2.8) {Safety gate\\vessel zones + force caps;\\10 kHz beat,\\$100\,\mu$s watchdog};
\node[mmin]  (vend) at (4.8,-2.8) {Robot vendor\\kinematic stack\\(isolated from LLM)};
...
\draw[mmedged] ($(gate.south)!0.4!(gate.south west)$) -- ++(0,-0.2) -| ($(act.south)!0.5!(act.south east)$);
\end{mermaidfig}
\vspace{-0.65cm}
...
$100\,\mu$s watchdog before any actuator moves. Actuator telemetry returns to the\\sensor stack, and every step is written to a hash-chained federated audit trail
...
\node[mmstep](a7)  at (8.4,-4.6)    {Arm 7 suction +\\irrigation (P1-P8)};
\node[mmstep](a8)  at (4.2,-4.6)    {Arm 8 imaging +\\drain (P1, P4, P8)};
...
stations, status displays, alarm systems, handoff procedures, and training
requirements for operating and supervising personnel across fleet.
...
end-to-side hepaticojejunostomy (0.20 to 0.50 N), and antecolic
gastrojejunostomy (0.40 to 0.80 N), guarded by Arm 5 ring-tension monitor
issuing a soft warning outside band.}
...
21 CFR part 812 with investigational-device caution statement and are not
relabeled or repackaged at site.
...
detectable and time-stamped. The audit trail also satisfies preservation
