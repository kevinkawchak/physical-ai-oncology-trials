...
\node[mmdec] (elig)at (0,-2.75)   {Eligible?\\\S5.1 / \S5.2};
\node[mmin]  (sf)  at (4.8,-2.75) {Screen failure\\(minimal data set, \S5.4)};
\node[mmstep](rand)at (0,-5.8)   {RCT 1:1 stratified, permuted-block\\consent, Physical AI opt-out};
\node[mmgoal](arma)at (-2.4,-8.2){Arm A (experimental)\\daraxonrasib RP2D\\LLM robotic Whipple};
\node[mmgoal](armb)at (2.4,-8.2) {Arm B (control)\\mFOLFIRINOX\\standard Whipple};
\node[mmstep](fu)  at (0,-10.4)  {Follow-up q8-12wk\\BICR RECIST 1.1\\central pathology};
\node[mmgoal](eos) at (0,-12.4)  {Endpoints\\PFS (primary)\\OS to 24 months};
\draw[mmedge] (scr) -- (elig);
\draw[mmedge] (elig) -- node[mmlabel,above, xshift=-0.06cm, yshift=0.08cm]{no} (sf);
\draw[mmedge] (elig) -- node[mmlabel,left, yshift=0.07cm, xshift=-0.09cm]{yes} (rand);
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 2. Overall schema for the Phase 2 multicenter RCT,
from 28-day screening through eligibility determination, central 1:1 stratified
randomization with Physical AI consent opt-out, the two parallel arms (Arm A
...
review; and the PFS and 24-month OS endpoints, with screen-failure branch.}
...
\label{tab:soa}
\begin{tabularx}{\textwidth}{L{4.0cm} C{1.7cm} C{1.35cm} C{1.0cm} C{1.3cm} C{0.85cm} C{0.85cm} C{1.6cm}}
\toprule
