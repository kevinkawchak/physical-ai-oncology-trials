...
\addlinespace
3. Compare clinically relevant postoperative pancreatic fistula between arms. & ISGPS grade B or C postoperative pancreatic fistula rate \cite{Bassi2017}. & Grade B/C fistula is the leading driver of post-Whipple morbidity and a key safety-quality discriminator between the two operations. \\
\addlinespace
...
\addlinespace
5. Compare molecular response between arms. & KRAS circulating tumor DNA (ctDNA) clearance at week 12 \cite{Tie2019ctdna}. & Week-12 ctDNA clearance is an early, minimally invasive molecular readout of disease control that complements the imaging-based primary endpoint. \\
\midrule
...
in turn sits above the estimation-only secondary measures and the exploratory
measures.

\begin{mermaidfig}
\node[mmgoal] (prim) at (0,0)       {PRIMARY\\PFS: Arm A vs Arm B};
\node[mmstep] (sec)  at (0,-2.6)    {KEY SECONDARY\\(fixed sequence)\\OS; R0; ISGPS B/C;\\MPR; ctDNA};
\node[mmin]   (oth)  at (-4.2,-5.4) {SECONDARY\\(estimation)\\Clavien III+; mortality;\\conversion; QoL};
\node[mmin]   (exp)  at (4.2,-5.4)  {EXPLORATORY\\LLM concordance;\\sim-to-real; federated\\model; cost};
...
and the exploratory measures (LLM advisory concordance, the sim-to-real gap,
