...
\node[mmstep](rand) at (0,-3.4)  {Randomized 1:1\\(n = 220, central\\permuted block,\\stratified)};
\node[mmgoal](armA) at (-3.7,-6.8){Arm A (n approx.\ 110)\\daraxonrasib RP2D +\\LLM robotic Whipple};
\node[mmstep](armB) at (3.7,-6.8) {Arm B (n approx.\ 110)\\mFOLFIRINOX +\\standard Whipple};
\node[mmdark] (di) at (6.9,-6.8) {Discontinued\\intervention\\(progression,\\withdrawal, \S7)};
\node[mmgoal](an) at (0,-10.0)   {Analyzed: ITT, mITT,\\per-protocol, safety\\(both arms)};
...
\draw[mmedged] (armA.south) -- ++(0,-0.4) -| ($(di.south)+(0,-0.4)$) -- (di.south);
\end{mermaidfig}
\vspace{-0.7cm}
...
intention-to-treat, modified intention-to-treat, per-protocol, and safety analysis
...
pancreaticobiliary surgery and to receive either study regimen.
...
during the study if applicable.
...
the daraxonrasib labeling and the concomitant-medication rules in \S6.
...
\cite{kawchak2026hr9510,ecfr2026part54}. Because the population is vulnerable,
