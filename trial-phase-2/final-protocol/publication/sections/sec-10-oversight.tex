...
\node[mmstep](irb) at (0,-4.2)    {Single IRB (sIRB)\\45 CFR 46.114};
...
group-sequential interim plan, convenes a Physical AI Safety Review Committee on a
$\leq 90$-day cadence, designates an Independent Safety Monitor for each procedure,
and delegates monitoring to a robotics- and AI-competent CRO over the site
...
consent and reaffirmed at each contact. Figure 20 shows the consent process and the
opt-out.
...
randomized to Arm A a documented right to request non-Physical AI administration
where clinically feasible (21 CFR $\S$312.60(f)), with the standard right to
withdraw.}
...
\node[mmstep](lev)  at (-4.6,-8.2)  {Permitted operational\\levers: 8 sites,\\Phase 0 compute, fleet,\\access fund, review core};
\node[mmgoal](ovr)  at (0,-8.2)     {Independent oversight\\sIRB, DSMB, Physical\\AI Safety Committee};
\node[mmdark](sci)  at (4.6,-8.2)   {Randomization,\\endpoints, adjudication,\\analysis, data,\\publication};
\draw[mmedge]  (fund) -- (fw);
\draw[mmedge]  (fw) -- (admin);
\draw[mmedge]  (admin) -- (lev);
\draw[mmedge]  (admin) -- (ovr);
\draw[mmedged] (fw.east) -- node[above,font=\scriptsize,pos=0.5]{blocked} (sci.north);
\node[font=\scriptsize\bfseries,text=protomaroon] at (3.05,-4.7) {no influence};
\end{mermaidfig}
\vspace{-0.55cm}
