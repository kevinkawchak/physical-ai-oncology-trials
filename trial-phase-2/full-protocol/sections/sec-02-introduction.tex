...
and preserves a complete resection. In Scenario C, manual mistiming of the
...
\node[mmdark](dw) at (5.2,-10.15){Under-funded, single-center,\\under-powered: no\\definitive equitable answer};
...
\draw[mmedge] (d0.east) -- node[above,font=\scriptsize, align=center, xshift=-0.075cm, yshift=0.315cm]{under-funded} (dw.west);
...
integrated approach worsens the patient or the evidence. Scenario A: a human-only
...
during manual surgery triggers the fistula-sepsis cascade; the no-fly gate with a
$\leq 3$ ms emergency stop averts it. Scenario C: manual mistiming of the
...
off from the endpoints by the capital firewall, converts capital into accrual
...
so that capital can never buy a favorable result. What capital can buy is limited
...
dropout and thereby protects statistical power, while making enrollment more
equitable and representative. Fifth, central infrastructure (BICR, central
...
\node[mmgoal](m1) at (5.3,0)      {Phase 0 sim validation;\\narrow locked device};
\node[mmgoal](m2) at (5.3,-1.6)   {10 kHz heartbeat, $\leq 3$ ms\\e-stop, force caps};
\node[mmgoal](m3) at (5.3,-3.2)   {Human-in-the-loop; surgeon\\plus backup plus ISM};
\node[mmgoal](m4) at (5.3,-4.8)   {Cross-framework\\($\geq 3$ frameworks)};
\node[mmgoal](m5) at (5.3,-6.4)   {On-premises LLM,\\open weights, isolated};
\node[mmgoal](m6) at (5.3,-8.0)   {Domestic deployment;\\data residency preserved};
\node[mmgoal](m7) at (5.3,-9.6)   {Deny-by-default,\\simplified MCP gate};
\node[mmgoal](m8) at (5.3,-11.2)  {Hash-chained audit\\trail; 21 CFR part 11};
\node[mmgoal](m9) at (5.3,-12.8)  {Capital firewall, equity fund,\\part 54, H.R. 9510, CC BY};
\draw[mmedge] (c1) -- (m1);
\draw[mmedge] (c2) -- (m2);
\draw[mmedge] (c3) -- (m3);
\draw[mmedge] (c4) -- (m4);
...
access, is answered by the capital firewall, the Patient Access and Equity Fund,
21 CFR part 54 disclosure, the H.R. 9510 VVUQ financial standard, and open CC BY
deposition.}
...
Phase 0 compute & $\geq 5000$ simulations across $\geq 3$ frameworks & Lower sim-to-real gap ($< 1.5$ mm, $< 0.4$ N) and USL $\geq 8.0$, raising procedural fidelity; compute cannot alter the analysis plan. \\
...
standard of care \cite{Conroy2018}. The contemporary robotic baseline quantifies
the residual magnitude of these operative risks even in expert hands
...
\begin{mermaidfig}
\node[mmdark](base) at (0,-0.65)  {Advanced PDAC baseline\\5-year OS $<$ 13\%\\3rd US / 4th EU death};
\node[mmin]  (r1)  at (-4.9,-2.4) {Device malfunction};
\node[mmin]  (r2)  at (-4.9,-4.4) {AI advisory error};
\node[mmin]  (r3)  at (-4.9,-6.4) {Cyber-physical failure};
\node[mmstep](b1)  at (4.9,-2.4)  {In-window R0 resection};
\node[mmstep](b2)  at (4.9,-4.4)  {Hard limits plus audit trail};
\node[mmstep](b3)  at (4.9,-6.4)  {Restart timing plus equity fund};
\node[mmdec] (w)   at (0,-4.4)    {Benefit-risk for\\low-survival patients};
\node[mmgoal](j)   at (0,-7.9)    {Favorable: benefits likely outweigh\\risks; \S312.84};
\draw[mmedge] (base) -- (w);
...
\begin{mermaidfig}
\node[mmin]  (cap) at (0,0)       {Aligned capital\\and PACIF tranches};
\node[mmdec] (fw)  at (4.5,0)     {Capital\\firewall};
\node[mmstep](l1)  at (9.0,2.55)  {8 academic\\HPB sites};
\node[mmstep](l2)  at (9.0,0.85)  {Phase 0 compute\\($\geq 5000$ sims)};
\node[mmstep](l3)  at (9.0,-0.85) {Patient Access\\and Equity Fund};
\node[mmstep](l4)  at (9.0,-2.55) {Central review\\core (BICR)};
\node[mmgoal](o1)  at (13.7,2.55) {Higher\\statistical power};
\node[mmgoal](o2)  at (13.7,0.85) {Lower\\sim-to-real gap};
\node[mmgoal](o3)  at (13.7,-0.85){Lower dropout,\\equitable enrollment};
\node[mmgoal](o4)  at (13.7,-2.55){Definitive, generalizable,\\equitable answer};
\node[mmdark](block) at (4.5,-3.3){Endpoints, adjudication,\\analysis: no funder path};
\draw[mmedge] (cap) -- (fw);
\draw[mmedge] (fw.east) -- (l1.west);
\draw[mmedge] (fw.east) -- (l2.west);
\draw[mmedge] (fw.east) -- (l3.west);
\draw[mmedge] (fw.east) -- (l4.west);
\draw[mmedge] (l1) -- (o1);
\draw[mmedge] (l2) -- (o2);
\draw[mmedge] (l3) -- (o3);
\draw[mmedge] (l4) -- (o4);
\draw[mmedged] (fw) -- node[right,font=\scriptsize,align=center]{blocked} (block);
\end{mermaidfig}
\vspace{-0.45cm}
