...
an amendment under \S312.30, and no participant is dosed before the amended IND
is in effect.
...
\node[mmstep](ind)   at (0,-2.4)    {Amended IND\\\S312.21(b)\\300 mg PO daily};
\node[mmin]  (sr)    at (4.6,0)      {Significant-risk\\IDE \S812.3(m)\\randomized controlled};
\node[mmstep](ide)   at (4.6,-2.4)   {IDE approved\\(FDA plus IRB)\\PancreSpeed II};
\node[mmstep](p0)    at (2.3,-4.8)   {Phase 0 sim validation\\$\geq 3$ frameworks\\$<1.5$ mm / $<0.4$ N};
\node[mmstep](usl)   at (2.3,-7.0)   {USL readiness $\geq 8.0$\\fleet harmonization\\federated audit};
\node[mmgoal](cls)   at (2.3,-9.2)   {Single IRB (sIRB)\\8 HPB sites\\45 CFR 46.114};
\node[mmgoal](enr)   at (2.3,-11.4)  {Multicenter randomized\\enrollment 1:1\\Class II, $\leq 500$ ms e-stop};
\draw[mmedge] (pre) -- (ind);
\draw[mmedge] (sr)  -- (ide);
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 1. Combined IND / IDE regulatory pathway with the upgraded
...
upgraded Subpart J overlay of Phase 0 simulation validation across at least three
