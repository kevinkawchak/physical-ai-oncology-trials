...
### Added
- `funding/auto-fund/` - **Daily Funding Actions for Final CEO Approval**, five business days of complete funding actions for CEO Kevin Kawchak (ChemicalQDevice), each carrying every address, subject line, body, form field, order type and limit the day needs, so that the remaining work on any day is one approval rather than a drafting session. The directory turns on one external change: the FDA approval of **Rasonque (daraxonrasib)** on August 26, 2026, the molecule this company selected in June 2025 and simulated in August 2025. The approval converts fourteen months of work from a prediction into a record; it does not make the perioperative use this program proposes approved, does not remove the investigational new drug application, and no file in the directory says otherwise.
- `funding/auto-fund/prompts/` - the master prompt filed verbatim (`prompt-auto-fund.md`) under a single heading and nothing else, and the full build output of record (`output-auto-fund.md`) under a single heading and nothing else.
- `funding/auto-fund/sub-prompts/` - one schedule per business day rather than a stage schedule, because this build produces five independent daily deliverables rather than three successive drafts of one document. `day-1-02Sep26` through `day-5-08Sep26`, each with its own commit order, its figure and table assignments, and the nine invariants it may not relax. No publication directory is generated anywhere, by instruction.
- `funding/auto-fund/inputs/` - an index of the nine repository sources every day reads, rather than an archive. Nothing is copied: every source already has a home in this repository, and a second copy would drift from the first.
- `funding/auto-fund/02Sep26/` - **Day 1, Approval to Ask.** Five federal re-contacts written against the approval, none of them a cold letter: each names the earlier July or August 2026 inquiry, states the one fact that changed, and asks a question the earlier letter could not have asked. Two registration form packs, a four-rung Treasury ladder cut to a nine-month SBIR Phase I horizon, three figure specifications, and *The Approval Dividend*, 14 pages. Two of the five produced substantive replies, and letters 6 and 7 answer them; each carries one purpose-built one-page attachment in addition to the compiled packet.
- `funding/auto-fund/03Sep26/` - **Day 2, The Private Capital Bridge.** Three candidate instruments at one raise size scored on eight attributes, with the SBIR ownership row that decides the choice under 13 CFR 121.702; the 21 CFR part 54 firewall each instrument must leave intact; the first approach of any kind to the agent's developer; two private-placement filings prepared and deliberately unfiled; and *The Private Capital Bridge*, 13 pages. No offering exists and no valuation, cap, discount, minimum or closing date appears anywhere, including in the figures.
- `funding/auto-fund/04Sep26/` - **Day 3, The Site and Partner Package.** Two La Jolla institutions approached in parallel on the same day by two different routes, each letter disclosing the other; two disease foundations asked a mechanism question rather than for money; the clinical trial support services request whose sponsor-investigator determination gates every other item; a site start-up reserve of $420,000 committed to nobody; and *The Site and Partner Package*, 12 pages.
- `funding/auto-fund/07Sep26/` - **Day 4, The Staged Queue.** Labor Day, with federal offices and both exchanges closed. Four letters written and held under a `HOLD FOR RELEASE` line, six orders sized and priced and present in no broker system, a nine-folder data room, the fixed wording for the three presidential recognition letters, a twenty-two question diligence bank with four questions marked as having no answer, and *The Staged Queue*, 13 pages.
- `funding/auto-fund/08Sep26/` - **Day 5, The Execution Record.** The queue released against six checks in a sequence an auction cutoff sets rather than one chosen freely; follow-ups sent only where three business days have elapsed and withheld where they have not; a thirty-day pipeline in which every row waits on somebody other than this company; a weekly cadence of five standing functions at seven and a half hours; and *The Execution Record*, 14 pages, whose closing table states twenty-one actions in rows that stand without any file being opened.
- `fundstyle.sty` - `capstyle.sty` from `funding/capitalization-plan` with five changes and no others: a `-0.60cm` spacing invariant giving exactly 7.44 pt from every float's last rule to the first caption line; two-line hand-balanced captions rather than three; a new cover that leads with the decision rather than with the money, `\fundmast` over `\funddeck` and `\actionbox`; a per-day palette confined to one block, so the five copies differ only in six hex values; and the deletion of the parent's `\umlactor` stick-figure macro, so no basic human stick figure can appear in a document a funder opens.
- `references.bib` - 56 entries shared byte-identically across all five packets. Thirty are carried unchanged from `funding/capitalization-plan/final-capital/references.bib`; the remainder are new. Every entry carries a resolvable target, every entry with a digital object identifier carries a paired `doi` and `url`, and the style is `unsrturl` rather than `unsrt`, because plain `unsrt.bst` reads neither field.
- Quantitative evidence sufficient for a funder, reused without re-derivation: the six checkable quantities each carrying the limitation its own authors stated in the same table row as the result; the $306,000 Phase I, $1,300,000 Phase II, $1,606,000 route, $2,104,000 delta, $5,900,000 private position and 3.67 to 1 leverage frame; the eleven-role roster at 3.95 award-funded full-time equivalents inside the $700,000 annual direct cost; and a projected $36,330 virtual trial against an industry benchmark above $120,000.
...
### Changed
- `funding/auto-fund/02Sep26/emails/` - two replies added as letters 6 and 7 after substantive responses came back from the Cancer Therapy Evaluation Program and from the High-Risk, High-Reward Research Program. The first is the more consequential: it establishes that a cooperative research and development agreement is entered by the drug company, which this company is not, and that all of that program's trials are investigator-initiated, so a company-originated concept is the wrong shape by construction. The recorded consequence is that the day 3 site conversation stops being one step in a sequence and becomes the precondition for that federal route, and this company's role narrows to the advisory software component and the documentation. Letter 6 concedes both points and asks for no exception. The second reply confirms two viable application routes and identifies the operative clause, which is the certification that the submitting entity will house and support the research rather than the registration mechanics; letter 7 asks what that certification means for a small business before making it, and contains no sentence about the research, per the boundary its recipient drew.
- `funding/auto-fund/02Sep26/packet/` - Table 2 extended from five rows to seven, a new subsection in §2 recording what the two replies established and what the first changes, the cover badge and subtitle updated, and the approval box extended to seven letters. Recompiled at 14 pages, 0 errors, 0 overfull boxes, no page under 20 body lines.
- Root `README.md`: release badge to v4.8.0, updated date badge, four new v4.8.0 badges, a dated 9/8 headline entry, one new section (*Daily Funding Actions for Final CEO Approval*) carrying two mermaid diagrams and five tables, and the `funding/` subtree updated with the `auto-fund/` directory. One new section, and the existing badge row, headline entry and structure tree updated in place rather than duplicated.
- `funding/README.md`: repository version to v4.8.0, two new badges, the `auto-fund/` directory added to the structure tree, and one new numbered section for v4.8.0 with the following sections renumbered.
...
### Fixed
- Two fatal TikZ defects diagnosed only from the compile log, both caused by generated node names: a `\foreach` over negative decimal coordinates produced names such as `st-2.76`, which TikZ reads as node `st-2` at anchor `76`, and an unbraced `fit` value carrying commas was split into separate keys by the key parser. Both classes are now recorded as an invariant in every `diagrams/README.md`, and every node name in the fifteen figures is plain letters and digits.
...
- `funding/auto-fund/*/emails/` - all 26 letters rewrapped so that a paste into a mail client needs no repair. The bodies had been hard-wrapped at about 80 characters, which is correct in a text editor and wrong in iOS Mail: the client keeps every hard break and then soft-wraps what is left, so the author had to delete line breaks and pull words up to close the ragged gaps before sending. Each paragraph is now one physical line and the client wraps it to the device width. 703 hard line breaks were removed from the bodies. The lines that are deliberately their own line are kept: the salutation, numbered and bulleted items, ALL-CAPS headers, separators, and the signature block. Two letters also carried monospace-aligned tables, which break in any proportional font: the release record's nine released items and three withheld items in `08Sep26/email-01-release-cover-note.txt`, and the meeting block, attendee list and five-question agenda in `08Sep26/email-04-site-meeting-confirmation.txt`. Both are now one item per line with the alignment carried by punctuation rather than by spaces. Eight `**` bold markers in four letters, which paste as literal asterisks, were removed. No word of any letter changed: the body word stream of all 26 files is identical to the version before this change, verified token by token. Only the body region was touched. The `FROM`, `TO`, `CC` and `SUBJECT` lines were already single lines, and the attachment lists and pre-send checklists below `=== END BODY ===` are the author's reference material rather than pasted text, so they keep their wrapped form and their one-URL-per-line layout.

## [4.7.0] - 2026-08-23

*Version 4.6.0 was delivered separately in pull request #76, which has since
merged into `main` and is carried here by that merge. This entry is additive
and does not modify it.*

### Added
- `funding/move-in/` - **La Jolla Move-In: Pancreatic Oncology Clinical Trial Site Complete Documentation Package**, Draft 1.0, La Jolla, San Diego, August 23, 2026, by Kevin Kawchak (ChemicalQDevice). Fifteen documents that stand up California's first pancreatic ductal adenocarcinoma large language model oncology clinical trial site, for a chief executive and ten coworkers, against a federal award of $700,000 per year for five years. It succeeds the eleven-document San Francisco package (v2.9.0, DOI 10.5281/zenodo.19176370), which described a twenty-four-hour general oncology site serving 168 participants a day with twenty-nine robot instances. This package is narrower and harder: one disease, one Phase 1 protocol, one building, eleven named people, and a funded budget that has to close.
- `funding/move-in/prompts/` - the master prompt filed verbatim (`prompt-move-in.md`) and the full build output of record (`output-move-in.md`), which records every defect found with its measured size, the two instructions that could not be followed literally, and what is not claimed.
- `funding/move-in/sub-prompts/` - one three-stage schedule, not two and not eight: `draft-move-in/`, `full-move-in/` and `final-move-in/`, five sub-prompts each. There are no diagram-platform stages, because Rule 3 of the master prompt forbids diagrams; where the parent build would have drawn a figure, this one writes a table.
- `funding/move-in/draft-move-in/` - the compiling skeleton: `main.tex`, `movestyle.sty`, `references.bib`, seventeen section files carrying 96 bracketed `[DRAFTING INSTRUCTION]` markers that name exact repository paths through `\mvfile`, two READMEs, and `draft-move-in-LaTeX.zip`. Compiles at 0 errors, 27 pages.
- `funding/move-in/full-move-in/` - every drafting instruction answered from the file it named and then deleted, all fifteen documents written, 56 full-width tables populated at the exact body measure, and `full-move-in-LaTeX.zip`. Compiles at 0 errors, 0 overfull boxes, 71 pages, with all 76 bibliography entries cited.
- `funding/move-in/final-move-in/` - the senior-author pass, driven by measurement rather than by reading: the stage 2 PDF was converted to text, every page counted, and every page carrying fewer than twelve body lines or ending on a heading was fixed at its cause. Compiles at 0 errors, 0 overfull boxes, 0 underfull boxes, 0 undefined citations, 0 undefined references, 0 short pages, 0 stranded headings, 67 pages. No `publication/` subdirectory, by instruction.
- `movestyle.sty` - `applystyle.sty` from `funding/pdac-funding-applications` with two changes and no others. Deleted: all five TikZ diagram vocabularies, the figure frame, the float carrier, the caption macro, the placeholder macro and the vector glyph macros, so a grep for `appfig` returns nothing. Added: `\docpart` and `\docfront`, which open a document with a ruled block, restart its section numbering, write a part-level contents line and re-key the hyperref anchors so fifteen documents that each begin at section 1 do not collide; the cover furniture; `\draftnote` and `\mvfile`; a numbered `\tabcap`; and `\mvltable`, the wrapper for a table that must break across pages.
- `references.bib` - 76 entries, all 76 cited from the body. Every entry with a digital object identifier carries a paired `doi` and `url` field, and the style is `unsrturl` rather than `unsrt`, because plain `unsrt.bst` reads neither field and a reference list built on identifiers would print none of them.
- The fifteen documents: four legislation and lobbying instruments (SB 1188 site authorization, AB 3162 participant rights and robotic safety, SB 964 model transparency and clinical data protection, H. R. 10412 the federal demonstration site act succeeding the author's own H. R. 9510); three regulations (San Diego Municipal Code update for the La Jolla Community Plan area, California Code of Regulations Title 22 Division 5 Chapter 15, and an FDA compliance guide mapping fourteen federal requirements to a document in the package); four building and premises documents; and four operations documents, of which document 13 is the load-bearing addition, a conventional pancreatic cancer Phase 1 requirements manual written to pass one test, that a reader who deletes every model and every robot from the site still holds a complete manual.
- Comprehensive READMEs with badges and Rule 5 source maps in all eleven directories under `funding/move-in/`, including the three `sections/` subdirectories and the `inputs/READMES/` subdirectory.

### Changed
- Root `README.md`: release badge to v4.7.0, four new v4.7.0 badges, a dated 8/23 headline entry, one new section (*La Jolla Move-In: PDAC LLM Oncology Clinical Trial Site*) with a mermaid build diagram and four tables, and the `funding/` subtree updated with the `move-in/` directory. One new section, and the existing badge row, headline entry and structure tree updated in place rather than duplicated.
- The single five-year budget frame of `$700,000` per year and `$3,500,000` over five years is reused verbatim from `funding/pdac-funding-applications/final-apply/sections/sec-08-budget-and-leverage.tex` and is not re-derived. It is split here into six lines that sum to `$700,000` exactly, with personnel at `$521,000` across eleven roles at 3.95 award-funded full-time equivalents.
- The `$36,330` figure for the author's virtual trial work is described as **projected** throughout, never as estimated, in document 13 §7 and in all three rows of the cost benchmark table.
- `funding/move-in/inputs/README.md`: the author's full deposited-abstract corpus, 131,220 characters covering 56 works, was moved without a single edit to `funding/move-in/inputs/READMES/README-ChemicalQDevice-Deposited-Abstracts-Corpus.md`, which is where the other input READMEs already live, and `inputs/README.md` is now the comprehensive directory README with badges that Rule O asks for. No content was lost.

### Fixed
- Four stage 1 defects, each recorded with its measured size: an unescaped underscore inside a repository path put the rest of a paragraph into math mode, producing 10 errors and three overfull boxes of 81.26 pt, 596.46 pt and 1622.11 pt; `\contentsname` held a color command that `\MakeUppercase` turned into an undefined color name, producing 3 errors; `\l@part` set `\rightskip` with no stretch, producing three underfull contents lines at badness 10000; and `\Urlmuskip` at the parent's `0mu plus 3mu` took almost all of a ragged-right line's stretch, printing links with a visible gap between every character.
- Two style hazards inherited from the parent that would recur in any adaptation of it: the parent's `\AtBeginDocument` redefinition of `\thebibliography` takes a parameter, and the LaTeX begin-document hook stores its argument in a macro body, so the literal parameter is read as a parameter of that body and the compile stops at `\begin{document}`; and `\theHsection` does not exist until `hyperref` loads, so document-keyed anchor redefinitions must follow the `\RequirePackage`.
- Fourteen stage 2 overfull boxes and two uncited bibliography entries: a bold header wider than its 0.7 cm column by 0.65 pt; a twelve-character word 21.42 pt too wide for a 1.3 cm column, ten occurrences; a hyphenated compound 4.55 pt too wide for a 1.7 cm column; two twenty-row tables producing overfull vertical boxes of 196.14 pt and 223.34 pt because `tabularx` cannot break across pages; and `13 CFR 121.702` and the first funding application present in the bibliography but never referred to.
- Ten stage 3 defects, all found by measuring the compiled PDF rather than by reading the source: a contents that ran to four pages with two lines on the fourth; a table caption alone on a page; two document-closing paragraphs alone on pages carrying two and three lines; five pages ending on a section heading, because a `\section` reserved 3.4 baselines and that reservation is satisfied by space a full-width table cannot use; a caption separable from its table; fourteen tables of ten rows or more unable to break across pages; an abbreviation table carrying 30 entries of which 13 never appeared in the body and two cells were blank; two bold header cells wider than their columns; and thirty-eight fixed columns wrapping deeper than their neighbors.
- Six paragraphs tightened by a clause each, in documents 00, 01, 12, 13 and 15. In every case the instrument is the first in the fix hierarchy, a sentence, and never a skip: no `\vspace` was added to the body anywhere in stage 3.
...
- `new-trial-system/final-new-trial/update-final/` - the second-prompt update, a complete self-contained Overleaf project that reads nothing from its parent, with `update-final-LaTeX.zip`. It sets in 53 pages with no error, no undefined citation, no undefined reference, no bibtex warning and no overfull box above 5 pt. This is the version to compile.
- `new-trial-system/communications/` - thirteen outbound plain-text messages, every one carrying the paper as its first attachment. Nine funding follow-up emails in `funding-emails/`, one per organization in `funding/pdac-funding-applications/applications/emailed-source`, each in the format of that organization's own `email-app-xx` file with its recipients carried forward, a body closing August 14, 2026, an ordered attachment list specific to the organization, and a `BEFORE SENDING` checklist. One LinkedIn post in `linkedin/`. Three brief general messages in `general/`, written from the patient and caregiver, clinical investigator, and policy and press perspectives.
- `new-trial-system/final-new-trial/update-final/trialstyle.sty` - a class diagram vocabulary added to the PlantUML section: `umlclshdr`, `umlclsbody`, their gray and key variants, `umlstereo`, `umlassoclbl`, and the `\umlclass` constructor, which draws a two-compartment class from two abutting nodes so the header fill, the member measure and the class height are each set exactly and the class carries a real anchor on every edge for association routing.

### Fixed
- **The stage 8 bundle did not compile.** Three pgf `\foreach` headers declared iteration macros with a digit in the name: `\x0` and `\x1` twice in `sec-03-ind.tex`, `\b1`, `\b2` and `\b3` in `sec-05-legislation.tex`, and `\p0`, `\p1` and `\d1` in `sec-06-funding-proposals.tex`. A TeX control sequence cannot carry a digit, so pdfLaTeX raised `Missing control sequence` inside Figure 7 and halted with no output file at all, which made every other property of the stage unverifiable. The headers are re-declared with letter-only names in `update-final`.
- **Figure 14 no longer uses stick actors.** The use case drawing terminated its thirteen association lines at stick-actor caption nodes rather than at the glyphs, so the lines did not appear to connect, and a five-stroke figure reads as elementary beside a bill amending the Federal Food, Drug, and Cosmetic Act. It is redrawn as a class diagram: six actor classes each carrying its own duties in a member compartment with the duty numbers and the prior-law verdict, joined by six labeled associations that run edge to edge, three inside a row and three inside a column, none crossing another or a class. The caption and the surrounding context are unchanged. The specification at `new-trial-system/plantuml/fig-14-statutory-actor-duties.md` is rewritten to match.
- Five defects found on the rendered pages, which had never been seen because the prior stage produced no PDF: Figure 2's emphasis callout overlapped its in-figure note across a 0.10 cm band; Figure 4's loop frame label sat on the Author activation bar; Figure 6 ran two inbound edges through the words of its own subject label, ran the regulatory edge across the whole simulation cluster, and ran the clinical edge through the device cluster's label row; Table 13's bold `Characters` header overflowed its 1.55 cm column by 8.19 pt; and two paragraphs each spilled a two-line orphan onto an otherwise empty page.

### Changed
- `new-trial-system/final-new-trial/update-final/sections/sec-07-ai-peer-review.tex` - the AI Peer Review section is rewritten around quantities rather than adjectives. Four costs the prior regime imposes: about 51,980 US pancreatic cancer deaths in 2025, about 142 a day, so roughly 7,400 inside one 7 to 8 week best-case round; a display-item ceiling of six to eight per article against this paper's 25 figures and 25 tables, each figure a 6.5 KB specification rather than an image; list article charges of $2,000 to $6,000 and above $10,000 at flagship titles against the source study's total spend of $35 across a 14-day workflow; and the direction the money runs, from the author to the journal, for a page on the journal's own site. Three the new regime removes: this paper finished within three days, August 12 to 14, 2026; major paper and diagram code revision on the 1 to 2 day scale; and the author's position that on the record from late 2025 to the present, human peer review is not required for this class of work, bounded explicitly so it does not touch IRB approval, an FDA information request or a DSMB. Table 21 is re-cut to nine axes and Figure 22's grid to seven of them in the same order. Table 22's model roster, Figure 23's concurrency and Figure 24's disagreement tree are carried forward unchanged: Anthropic producing, OpenAI first reviewer, Google second reviewer, human PI holding final authority.
- `new-trial-system/final-new-trial/update-final/sections/sec-03-ind.tex`, `sec-04-trial-protocol.tex`, `sec-06-funding-proposals.tex` - each gains a closing paragraph tying its artifact to the review clock of section 7.
- `README.md` - v4.6.0 release badge, four new badges, a dated headline entry with a 425-character summary, one section carrying a mermaid build diagram now showing the second prompt and the update stage, two new tables for what the update changed and what section 7 measures, and `update-final/` and `communications/` added to the repository structure. The v4.5.0 summary's one non-US spelling is corrected to `program`.
- `releases.md` - v4.6.0 release notes in the required format.
- `new-trial-system/README.md`, `new-trial-system/prompts/README.md` and `new-trial-system/final-new-trial/README.md` - each records the second prompt, and the stage 8 README opens with a notice that its own bundle does not compile and that `update-final` is the directory to compile.
...
### Added
- `funding/capitalization-plan/` - the ChemicalQDevice company-conversion paper *From Independent Scientist to Novel Performer: A Small-Business Operating, Milestone, and Capitalization Plan for a Phase 1 LLM-Advised Robotic Whipple (ChemicalQDevice)*, Draft 1.0, San Diego, August 11, 2026, by Kevin Kawchak (ChemicalQDevice). Ten funding applications were filed in August 2026 as an independent scientist; nine address mechanisms that fund a person or an institution, and one, application 05, addresses the only mechanism in the set built for a company. This release converts the author to a small-business operator and rewrites that application as an operating, milestone and capitalization plan. The clause it turns on is the SBIR clause of *Science: A New Golden Age*, which appears in Chapters I, III and IV and is the only clause in the report written for a firm, rather than the report's individual-scientist language or its NOVEL PERFORMERS chapter.
- `funding/capitalization-plan/prompts/` - the master prompt filed verbatim (`prompt-capital.md`), the update prompt filed verbatim in the same form (`update-capital.md`), and the full build output (`output-capital.md`), which records the eight defects found and fixed with their measured sizes and the two instructions that could not be followed literally.
- `funding/capitalization-plan/sub-prompts/` - one eight-stage schedule, not two: five diagram-platform stages then draft, full and final. One directory and one comprehensive README per stage.
- `funding/capitalization-plan/mermaid/`, `plantuml/`, `d2/`, `diagrams-python/`, `graphviz/` - twenty new figure specifications in an uneven 5/3/5/3/4 split chosen by purpose rather than quota. Each carries a three-line balanced caption, valid source in its native platform syntax, TikZ construction notes with stated pitches and absolute coordinates, an anti-defect record against all five defect classes, and the repository files it draws on.
- `funding/capitalization-plan/draft-capital/` - the twelve-section skeleton with `[DRAFTING INSTRUCTION]` markers naming exact repository paths through `\appfile`, twenty sized figure slots, and `draft-capital-LaTeX.zip`. Compiles at 0 errors, 32 pages.
- `funding/capitalization-plan/full-capital/` - every instruction resolved, all twenty figures drawn in TikZ, twenty-one full-width tables populated from author sources, and `full-capital-LaTeX.zip`. Compiles at 0 errors, 0 overfull boxes, 43 pages.
- `funding/capitalization-plan/final-capital/` - the senior-author pass: `\clearpage` discipline, the author's ten-item correction list from `useredits.md` applied item by item, every figure improved once more, and `final-capital-LaTeX.zip`. No `publication` subdirectory, by instruction. Compiles at 0 errors, 0 overfull, 0 underfull, 0 undefined citations, 0 undefined references, 44 pages.
- One update pass over stage 8, driven by `funding/capitalization-plan/prompts/update-capital.md`. Every one of the 41 captions now opens with its own number, `Figure N.` or `Table N.`, on three centered lines balanced to the narrowest spread its word boundaries allow; every figure and every table is referred to by number at least once in the running text; and `main.pdf` and `final-capital-LaTeX.zip` are rebuilt in one pass from one source set, so neither can be newer than the other. The $36,330 figure for the three prior work products is described as projected rather than estimated, in `sec-02` and in all three rows of Table 17.
- `capstyle.sty` - `applystyle.sty` with three changes and no others: a figure spacing invariant retuned so `\vspace{-0.65cm}` against a rigid 24.5 pt skip leaves a constant 6.06 pt caption gap for every figure and every table; a new cover of a ruled panel with a top accent band over a three-cell money ledger, varying from both the RFA-RM-27-001-v2 form-field theme and the parent's ten-cell recipient strip; and the author's `\DiagramXShift` carrier.
- `references.bib` - the parent bibliography extended with nine codified and policy entries this paper needs: 21 CFR part 54 with the FDA financial-disclosure guidance and Forms 3454 and 3455, 13 CFR 121.702, 2 CFR 200.414, the SBA SBIR policy directive, the NIH SEED program, the NIH Grants Policy Statement fee provision, 21 CFR 312.57 and 312.62, and the ten-application parent work. All 41 entries are cited from the body.
...
### Changed
- Root `README.md`: release badge to v4.5.0, four new v4.5.0 badges, a dated 8/11 headline entry, one new section (*ChemicalQDevice Capitalization Plan*) with a mermaid build diagram and four tables, and the `funding/` subtree updated with the `capitalization-plan/` directory.
- The single five-year budget frame of `$700,000` per year and `$3,500,000` over five years, split across four layers of `$1,600,000`, `$720,000`, `$780,000` and `$400,000`, is reused verbatim from `funding/pdac-funding-applications/final-apply/sections/sec-08-budget-and-leverage.tex` and is not re-derived anywhere in the new paper.
- @kevinkawchak updated DOI ’xxxxxxxx’ identifiers to ‘21887807’ for physical-ai-oncology-trials/blob/main/funding/capitalization-plan/README.md and main README.md on 2026-08-11.

### Fixed
- Three markdown diagram defects elsewhere in the repository, found by scanning all 1,096 markdown files for unbalanced fences and broken diagram blocks. `competitions/inputs/paper-a/chunk_10_bibtex_references.md` closed its bibtex fence on the same line as the final brace, so the fence never closed and every line after it rendered as code. `trial-phase-2/sub-prompts/prompt-1-mermaid.md` and `trial-documents/sub-prompts/prompt-1-mermaid.md` each began a line at column zero with a three-backtick mermaid literal; because a fenced block can interrupt a paragraph, that opened a fence and swallowed the rest of the document. Both now use the four-backtick literal form that `trial-ind` already used correctly.
- Eight LaTeX defects found and fixed inside the build rather than carried forward: a cover ledger 14.34 pt past the measure; four table columns whose bold header proved wider than any body cell in them; eleven digital object identifiers overflowing a 4.0 cm column because `\dlink` text is unbreakable; a monospace directory path overflowing by 11.23 pt because Courier offers no break point inside a path; a TikZ node width written as `{\w*0.415} cm`, which the mathematics engine reads as a single expression and rejects; a `\foreach` loop with an empty body; a 143 pt overfull vertical box where a figure, a table and a closing paragraph competed for one page; and a table index that counted nineteen tables where the paper carries twenty-one, with sixteen cross-references renumbered against the corrected index.
- Three further defects found in the update pass by measuring the compiled PDF rather than by reading the source. Horizontal centering: `capstyle.sty` centered a figure frame and a caption with a leading and a trailing `\hfil`, and TeX deletes a glue item that ends a paragraph, so the trailing one never survived to stretch and every float in the paper sat 13.1 pt right of the page center. All three carriers now close with `\null`, and the twenty frames measure 306.00 pt against a 306 pt page center with the forty-one captions within 0.53 pt. Bibliography: `unsrt.bst` reads neither the `doi` nor the `url` field, so a reference list built on 20 DOIs and 37 URLs printed none of them and offered nothing to click; the style is now `unsrturl`, the urlbst companion of `unsrt`, and printed DOIs and URLs are marked in the accent color. Figure order: the tier stack and the firewall in §4, and the milestone activity and the fault tree in §5, were numbered against the order they were drafted in rather than the order they are printed in, so the compiled paper put Figure 11 a page ahead of Figure 10 and Figure 15 a page ahead of Figure 14. The four were renumbered, and the two indexes, the stage table, the four specification files and their names, the five directory READMEs, three stage sub-prompts and every cross-reference moved with them.
- `.gitignore` now excludes LaTeX build artifacts (`*.aux`, `*.log`, `*.out`, `*.toc`, `*.bbl`, `*.blg`, `*.fls`, `*.fdb_latexmk`), and the artifacts already tracked under `funding/capitalization-plan/` were removed from the index.
...
- @kevinkawchak added respective application pdf, and application source code links to respective physical-ai-oncology-trials/tree/main/funding/pdac-funding-applications directories on 2026-08-08.
