...
### Added
- new-trial/national-24-7-trial/paper/full-paper/charts/ - 30 publication-quality matplotlib chart pack at 300 DPI for the v3.6.0 Accelerated Patient Prediction full paper, with a 50/50 mix of v3.6.0 ASCII / table replacements (15) and entirely new figures (15); 10 of 30 are full page sized for US letter portrait or landscape
- new-trial/national-24-7-trial/paper/full-paper/charts/README.md - Chart pack documentation with a 30-row image inventory mapping each PNG to its parent paper section, replacement target (or NEW), full-page disposition, and single-line caption; includes the FDA RTCT 28 April 2026 differentiation block that highlights robotics integration, 1M token predictive context, multi-perspective coverage, hourly commit cadence, and Core i5-6200U local verification across the 30 charts
- new-trial/national-24-7-trial/paper/full-paper/charts/instructions/ - 30 .md instruction files (one per chart) specifying purpose, source paper section, image properties (filename, DPI, size, palette, layout), source data tables, style rules (single dashes only, the section sign for SS where relevant, light mode only), and the suggested caption
- new-trial/national-24-7-trial/paper/full-paper/charts/scripts/ - 30 matplotlib .py scripts (one per chart) that each render their PNG at 300 DPI without external data inputs; numbers are embedded inline so a reviewer can audit each figure against the paper text without chasing CSV files; all scripts pass ruff check and ruff format --check under the repository's configuration
- new-trial/national-24-7-trial/paper/full-paper/charts/images/ - 30 PNG renderings at 300 DPI; charts 01, 07, 08, 10, 11, 15, 16, 18, 20, 24, 27 are full page (10 figures); the remaining 19 are sized for half-page or two-up placement
- 15 v3.6.0 ASCII / table replacements: chart 01 site network (replaces hour-00 facility ASCII + tab:sim1-hour00-network), chart 02 patient flow Gantt (replaces hour-12 patient flow ASCII), chart 03 robot status heatmap (replaces hour-23 robot status ASCII), chart 04 patient journey timeline (replaces 1,120 day journey ASCII), chart 05 stage USL trajectory (replaces tab:sim2-stages), chart 06 agent layers wheel (replaces tab:sim3-layers), chart 07 combined hourly workload (combines hour 00 + 12 + 23 sponsor workload tables per project brief), chart 08 daily metrics dashboard (replaces tab:sim4-daily), chart 09 local verification card (replaces local verification ASCII), chart 10 FDA extension chart (replaces tab:disc-fda-extension), chart 11 AI baseline comparison (replaces tab:disc-baseline-vs-sim), chart 12 cloud vs local trade-offs (replaces cloud-vs-local ASCII), chart 13 code vs text comparison (replaces code-vs-text ASCII), chart 14 Track A vs Track B trade-offs (replaces tab:future-tracks), chart 15 cross-simulation synthesis (replaces cross-simulation ASCII)
- 15 entirely new figures not present in v3.6.0: chart 16 FDA RTCT capability radar (full page), chart 17 cost savings waterfall (1.30 billion baseline to 0.91 million per-patient run with FDA 390 million to 650 million reduction band), chart 18 patient safety pipeline funnel (full page; 1,000-cohort flow), chart 19 1M token value proposition wheel (cross-modal, hourly cadence, repository scale, multi-perspective, local-friendly, auditable trail), chart 20 financial assessment dashboard (full page; KPI cards + per-stage savings + per-simulation cost), chart 21 PSL trajectory (63.4 to 70.0 across 168 hours), chart 22 multimodal inputs diagram (six modalities to 1M token context to four output artifact types), chart 23 commit cadence timeline (168 commits in 7 days vs 6 supervised baseline papers in 5 years), chart 24 safety vs efficacy quadrant (full page; four sims as squares, supervised as circles, foundation as triangles), chart 25 21 CFR / ICH compliance wheel (Sim 2 stages outer + Sim 3 sponsor layers inner), chart 26 robot authorization flowchart (Sim 4 day-1 168 clearances + 6 borderline forwards), chart 27 site / sponsor architecture (full page; sites + Paradigm Health + sponsors + FDA RTCT), chart 28 future roadmap (12-month two-swimlane Track A + Track B), chart 29 artifact counts treemap (Sim 1 392 + Sim 2 54 + Sim 3 176 + Sim 4 868 = 1,490 artifacts), chart 30 RTCT signal flow (sites + Paradigm Health channels + FDA RTCT API)

### Changed
- README.md - Updated version badge to v3.7.0, added the v3.7.0 chart pack entry above the v3.6.0 full-paper entry, added the Accelerated Patient Prediction Chart Pack architecture diagram block above the v3.6.0 architecture diagram, and added the new charts/ subdirectory to the repository structure
- CHANGELOG.md - Added v3.7.0 entry above v3.6.0
- releases.md - Added v3.7.0 release notes above v3.6.0
- ruff.toml - Added a per-file-ignore for new-trial/national-24-7-trial/paper/full-paper/charts/scripts/*.py (F401, F841, E402) to keep the lint-and-format CI workflow green on Python 3.10, 3.11, 3.12; the new chart scripts use heavy local style helpers and inline color tables that intentionally reference a small number of unused F401 / F841 / E402 patterns, and relaxing those rules for this specific subdirectory keeps the rest of the repository's stricter ruff configuration intact

### Notes
- All 30 chart scripts execute end-to-end on Python 3.10+ with matplotlib and numpy as the only dependencies, produce 30 PNG files at 300 DPI, and pass ruff check and ruff format --check under the repository's configuration
- Charts use light mode only (white or off-white backgrounds, dark text on light fills); no dark mode anywhere
- Single dashes only across every chart text element; no em dashes, en dashes, double dashes, or triple dashes; the section sign U+00A7 is used wherever the paper source uses the placeholder string SS as a section reference (none of the charts uses SS as anything other than the standby robot status code in chart 03)
- FDA RTCT 28 April 2026 differentiators are highlighted across multiple charts: 116 robot instances (charts 01, 03, 16, 26), 1M token predictive context (charts 11, 19, 22), multi-perspective coverage (charts 15, 27, 30), hourly commit cadence (charts 23, 28, 29), Core i5-6200U local verification (charts 09, 12, 14)
- The v3.7.0 chart pack does NOT modify or replace any file in the v3.6.0 full paper; it lives in a new charts/ subdirectory inside the v3.6.0 full-paper/ tree per the project brief; the v3.6.0 LaTeX source remains untouched
