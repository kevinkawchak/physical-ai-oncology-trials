...
*Version 4.6.0 is delivered on a separate branch in pull request #76 and is not
part of this release. This entry is additive and does not modify it.*
...
- `references.bib` - the parent bibliography extended with nine codified and policy entries this paper needs: 21 CFR part 54 with the FDA financial-disclosure guidance and Forms 3454 and 3455, 13 CFR 121.702, 2 CFR 200.414, the SBA SBIR policy directive, the NIH SEED programme, the NIH Grants Policy Statement fee provision, 21 CFR 312.57 and 312.62, and the ten-application parent work. All 41 entries are cited from the body.
...
- Three further defects found in the update pass by measuring the compiled PDF rather than by reading the source. Horizontal centring: `capstyle.sty` centred a figure frame and a caption with a leading and a trailing `\hfil`, and TeX deletes a glue item that ends a paragraph, so the trailing one never survived to stretch and every float in the paper sat 13.1 pt right of the page centre. All three carriers now close with `\null`, and the twenty frames measure 306.00 pt against a 306 pt page centre with the forty-one captions within 0.53 pt. Bibliography: `unsrt.bst` reads neither the `doi` nor the `url` field, so a reference list built on 20 DOIs and 37 URLs printed none of them and offered nothing to click; the style is now `unsrturl`, the urlbst companion of `unsrt`, and printed DOIs and URLs are marked in the accent colour. Figure order: the tier stack and the firewall in §4, and the milestone activity and the fault tree in §5, were numbered against the order they were drafted in rather than the order they are printed in, so the compiled paper put Figure 11 a page ahead of Figure 10 and Figure 15 a page ahead of Figure 14. The four were renumbered, and the two indexes, the stage table, the four specification files and their names, the five directory READMEs, three stage sub-prompts and every cross-reference moved with them.
