...
\draw[gvundir] (5.28,-1.27) -- (dec.north east);
\node[font=\tiny,text=fundink,fill=fundwhite,inner sep=1pt] at (3.05,-1.62) {alone is decisive};
...
\subsection{What a reader should do with this section}

Take the four rows of Table 19 first and ask, of each, whether the route already
in motion is the right one. Three of the four are questions to a federal office
or an institution and cannot be accelerated by this company. The fourth, the
robotic configuration, is settled by a site agreement and therefore moves at the
speed of \S2 of the preceding day rather than at the speed of a regulator.

Then take Figure 12 and ask a different question: not whether each leaf is
likely, but whether the second route under each pair is real. A second clinical
site was approached in parallel, and a second class of funder was approached on
the same day, so two of the four leaves have a live alternative today. The
decisive leaf does not, and that is the honest reading of this figure.
