...
A credibility assessment aligned to ASME V\&V 40 and to ICH M15 returned 81.9
across 55 verification notebook tests \cite{daraxsims,asmevv40,ichm15}. That is
a measured figure from the author's own suite, not a regulatory finding.

% The cost table, its caption and the paragraph that closes the document run to
% seventeen lines.  Reserving the block moves the three together: without the
% reservation the closing paragraph breaks onto a page of its own.
...
\$36,330 figure is a projection from the author's own source set at one operator
working sixty hours a week, and is reported as projected rather than as a
completed accounting.}

A funder should read this section before the model governance sections, because
the decisions above are the ones a data monitoring committee examines: a
dose-limiting toxicity window that spans the operation, four named grading
instruments, an eighteen-participant ceiling that follows from the escalation
rule rather than from a budget, and four sources each carrying its limitation.
