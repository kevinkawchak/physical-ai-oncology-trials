...
\textbf{06, Foundation for the NIH.} A narrow pre-competitive question no single
party owns: what happens pharmacologically when a RAS(ON) inhibitor is given
around a curative-intent pancreatic resection. Three parties contribute, and the
one shared output, a specimen-level pharmacodynamic dataset, is released openly
and owned by none of them. That is what makes it pre-competitive: it
differentiates no product.
...
appointment, and asks the eligibility question directly. The programme is
written as a state machine rather than a set of aims, and every failure
transition re-enters it: if exposure is inadequate the schedule changes and the
agent is retained; if suppression is not demonstrated the agent changes and the
question is retained.

\textbf{08, NCI Cancer Therapy Evaluation Program.} The most clinically
technical of the ten, because a concept review is a technical review. One design
point is put forward: an adverse event in this study may be attributable to the
drug, to the operation, or to the advisory system, and the dose decision depends
on which. Attribution is adjudicated by a blinded reviewer before the toxicity
window closes, and only the drug branch reaches the dose decision.

\textbf{09, Convergent Research.} The only application whose organization states
its own end date. Three properties are offered for testing: it closes in year
five with no year six budgeted; three releases are scheduled of which only the
specimen dataset depends on the trial succeeding; and every artifact has one
recipient class fixed in year one, because negotiating recipients in year five
is how a dissolved organization strands its outputs.

\textbf{10, UC San Diego Moores Cancer Center.} Different in kind: addressed to
the partner institution rather than a funder, asking for a 45-minute feasibility
meeting rather than money, and carrying three positioning corrections from the
author's own site research.
