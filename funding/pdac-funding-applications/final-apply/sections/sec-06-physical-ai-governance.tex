...
advisory path holds write access to the command channel. The operating surgeon
approves every motion \cite{onpremwhippl}.

\textbf{Stopping is bounded and measured.} Arm-level stop is specified at 3
milliseconds and system-wide stop at 500 milliseconds, both measured on the
...
recommendation. Conversion ordered for patient safety is recorded as clinically
appropriate; conversion caused by device, advisory, or integration failure
counts separately against the feasibility endpoint \cite{phase1ind}.
...
sequence gives the conversation, which is what an operating team recognises; the
state machine gives the guards, which is what a reviewer checks. A sequence with
no guards cannot say what is forbidden, and a state machine with no messages
cannot say who speaks.
...
\end{appfig}
\vspace{-0.7cm}
\figcaption{One operative day as a message sequence. Every arrow reaching\\
the robot leaves a human first, and the two audit writes are\\
the only messages that cross the hospital trust boundary.}
\end{appfloat}
...
\end{appfig}
\vspace{-0.7cm}
\figcaption{One top event and every path to it. Three of the four branches are AND\\
gates, so no single failure reaches the patient, and the one OR branch\\
is the one governed by procedure rather than by hardware.}
\end{appfloat}
...
\end{appfig}
\vspace{-0.7cm}
\figcaption{Six tables and the five keys that join them. Every released record can\\
...
The documents this paper summarises were produced by a small set of frontier
models with separated roles, and the separation is what makes the review chain
meaningful: no model reviews its own output \cite{repo2026}.
