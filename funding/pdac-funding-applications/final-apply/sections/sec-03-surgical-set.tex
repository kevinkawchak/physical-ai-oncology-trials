...
In the five surgical applications the operation leads. Daraxonrasib enters as
the perioperative arm the operation is timed around rather than as the headline,
and the endpoint table is led by conversion, anastomotic integrity, operative
time, blood loss, R0 margin, and 90-day morbidity graded to the international
definition \cite{bassi2017isgps}.

That ordering is not cosmetic. A mechanism office asks whether the hard part is
buildable; a clinical programme asks whether it is survivable. Set A answers the
first, and the hard part it names is the operation with a language model in the
room.

\subsection{Who Is Authorised to Do What}

The most common objection to a language model in an operating theatre is that
the boundary is asserted rather than specified. Applications 02 and 10 each
carry a three-actor slice. Figure 5 gives the whole set, including the two
actors no application figure contains: the research pharmacist and the
...
\end{appfig}
\vspace{-0.7cm}
\figcaption{Seven actors and the eleven actions each may take. Every action has at\\
least one human association; the advisory model has exactly one, and\\
the struck links are the four it is denied by construction.}
\end{appfloat}

Three fills separate three authority classes: Corporate Blue for the actions
only the operating surgeon takes, pale blue for shared clinical and oversight
actions, gray for the three the model may take, all of which produce text or
read data and none of which moves anything. A reader who ignores the labels
still reads the structure correctly.

\subsection{Applications 01 to 05}

\textbf{01, NIH Director's Pioneer Award.} The unit of risk is the individual,
not the aim. The bet does not survive decomposition into three project aims,
because the drug schedule, the operative plan, and the advisory boundary are
only interesting together.

\textbf{02, ARPA-H.} A gated programme rather than a project narrative, because
the report endorses the ARPA model on the ground that a program manager can end
a bet. Three gates, each with a numeric kill criterion checkable from programme
data rather than from a progress report.

\textbf{03, NSF TIP X-Labs.} Argues organization type rather than money. Five
activities must be held together and no existing performer type covers all five,
which is the gap the report says X-Labs was created to fill.

\textbf{04, DOE Genesis Mission.} The FY 2028 annex defines the Robotics mission
as ``general-purpose autonomous systems capable of dexterous manipulation,
mobility, and reliable operation in real-world environments, to initiate the era
of physical AI-driven scientific discovery''
\cite{kratsiosvought2026fy2028priorities}. A human operating room is the hardest
available instance of that sentence, and three artifacts are offered to the
mission rather than to the clinic \cite{eo14363genesismission}.

\textbf{05, NIH SEED SBIR.} The only one where the commercial product carries
the argument. The deliverable is not the robot and not the drug but the
...
The asymmetry between the first two gates and the last three is why every
attachment settles credibility and plausibility in its first two sections,
before any ask appears: a reviewer unconvinced there never reaches the budget.
