...
In July 2026 the White House asked the federal research portfolio, approximately
\$200 billion annually, to prioritise the individual scientist over legacy
institutions \cite{kratsios2026sciencegoldenage}. This paper reports what happens
when one unaffiliated scientist takes that literally and writes ten funding
applications for the same Phase 1 pancreatic cancer trial, each addressing the
mechanism its recipient runs.

The trial is constant across all ten: an open-label, single-arm, 3+3 escalation
of perioperative daraxonrasib (RMC-6236) in up to 18 participants with
resectable or borderline-resectable KRAS G12-mutated pancreatic ductal
adenocarcinoma, performed as a staged eight-arm robotic pancreaticoduodenectomy
with an on-premises language model confined to advisory output
\cite{onpremwhippl,phase1ind}. Five lead with the operation and five with the
drug; both sets describe the same hybrid procedure. UC San Diego Moores Cancer
Center is named in all ten as the intended partner of choice, at the feasibility
stage only, and described nowhere as a partner, sponsor, site, or endorser.

The evidence is stated with its limits attached. An August 2025 quantitative
systems pharmacology simulation returned median overall survival of 12.8 months
against 5.4 for chemotherapy \cite{qspmetpancre}; in May 2026 RASolute 302
reported 13.2 against 6.6 months in the RAS G12 metastatic population
\cite{rasolute302}. That proximity is chronology, not validation, and neither
result speaks to the resectable setting, which is what the trial would measure.

The applications carry no digital object identifiers. This paper carries one.

\bmhead{How to Read This Paper}

This is a summary of ten application file sets, not an application. Every claim
in it can be checked against a named file in a public repository
\cite{repo2026}.

It is not medical or regulatory advice, and not a claim that any institution,
sponsor, or drug developer has agreed to anything. No patient has been treated.
The trial does not exist until a site, an institutional review board, an active
investigational new drug application, monitoring, insurance, and a sponsor of
record are all in place.

Three reading paths. A \textbf{funder} should read \S2 and \S8. A
\textbf{clinician} should read \S6 and \S5. A \textbf{metascience reader}
should read \S1 and \S9.
...
would have forced at least four figures into a vocabulary that cannot state
their subject.
