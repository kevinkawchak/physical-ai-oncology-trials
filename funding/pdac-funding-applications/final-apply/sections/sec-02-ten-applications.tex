...
A recipient qualifies only if \textit{Science: A New Golden Age} names its
programme or the mechanism that programme runs. Nothing was included on
plausibility. The rule is what makes the set an argument rather than a mailing
list: every application opens by quoting the recipient's own mechanism back to
it, in the administration's wording, and then answers it.

Three families emerged: \textbf{person-based}, where the report names the
Pioneer Award by title and cites HHMI-style support as the evidence;
\textbf{organization-type}, where it names X-Labs, focused research
organizations, the ARPA program-manager model and SBIR; and
\textbf{partnership and cost share}, where it holds up FNIH and the Accelerating
Medicines Partnership as its model, names the Genesis Mission's Robotics line,
and asks agencies to prioritise non-federal cost share
...
\end{appfig}
\vspace{-0.7cm}
\figcaption{Ten applications on four attributes. The science column is\\
constant by construction: the same trial is described ten\\
...
What is constant is the trial. All ten describe the same population, the same
open-label single-arm 3+3 escalation to 18 treated participants, the same
co-primary endpoints, and the same advisory boundary at 3 milliseconds per arm
and 500 milliseconds system-wide \cite{onpremwhippl,phase1ind}.

That constancy is the point. A reviewer comparing two applications from this set
is comparing two funding mechanisms applied to one piece of science with the
science held fixed. It is the closest thing an individual applicant can build to
a controlled comparison of mechanisms, and it is possible only because the
protocol was published before the applications were written.

\subsection{Redundancy Across the Set}

Sending ten applications is not ten times one application. Because they fund
overlapping activity sets, most of the programme survives any single refusal.
Figure 6 makes that concrete and identifies the two activities that do not.
...
\end{appfig}
\vspace{-0.7cm}
\figcaption{Ten awards against eight trial activities. Six activities have\\
two or more upstream awards and survive any single refusal; the\\
two that do not are drawn heavier, and both are regulatory.}
\end{appfloat}

Six of the eight activities have two or more upstream awards: the interlock rig
from applications 02, 03 and 05; the logging schema from 03, 04, 05 and 09;
cohort accrual from 01, 02, 06, 07 and 08.
...
scarce resource is institutional standing, not funding, and the one application
that could supply it is the one with nothing to offer in return.
