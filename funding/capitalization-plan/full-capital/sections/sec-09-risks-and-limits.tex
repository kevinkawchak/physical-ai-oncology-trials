...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Ten risks, the halt point or milestone each attaches to, and whether the
mitigation exists today or is merely planned. Three rows read none today or
partial, and those three are the honest content of the table.}

Three rows carry no mitigation that exists today, and they are worth reading
together. Two of the three are the same thing seen twice: the plan depends on
one institution and holds no alternative. The third, the verification re-run, is
unfunded because it sits in the \$2,104,000 that the SBIR route does not reach,
which is exactly the kind of consequence \S3 exists to make visible rather than
to bury.
...
Figure 14 makes them visible, and a reader who has skipped the figure should
...
are knowable in advance and both are measured before they can do damage, and
that is the only reason a schedule with two single points of failure is worth
putting in front of a program officer at all.
