...
% FINAL stage improvements: Figure 4 gains a dated baseline strip so the
% seven-month asymmetry is visible inside the figure; Figure 5 gains a primary
% and foreign key legend; Figure 6 gains an explicit count on each boundary.
...
thirty-three months this plan covers. What the entity holds today is drawn in
Figure 4 as four zones, one of which is deliberately empty.
...
\node[d2cellg,minimum width=64mm,text width=61mm,minimum height=6mm,
      font=\tiny\sffamily,anchor=north west] at (0,-8.75)
  {13 January 2026 baseline: owned 2, licensed 3, contracted 0, absent 7};
\node[d2cellk,minimum width=64mm,text width=61mm,minimum height=6mm,
      font=\tiny\sffamily,anchor=north west] at (6.9,-8.75)
  {11 August 2026: owned 13, licensed 3, contracted 0, absent 7};
\node[font=\tiny\itshape,text=protogray,anchor=north west,text width=132mm]
  at (-1.3,-9.65) {The contracted zone is drawn at the size a three-tile zone
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 4. Four zones and thirteen tiles. The owned zone holds everything\\
...
Thirteen assets are owned outright, every one of them deposited and addressable,
and Table 6 lists each with the identifier a reader can resolve it by.
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 6. Thirteen owned assets, every one deposited and addressable. No row\\
carries an encumbrance, no row is subject to a licence granted to a third\\
party, and no row was produced under a grant that reserves rights in it.}
...
virtual trial, and the digital-twin platform were each produced for a projected
...
The \$36,330 is a projection from the source set and is reported as such, not as
an audited account; Table 17 carries it with its benchmarks.
...
phase2proto,phase1ind}. Four further rows are documents about how the work is
run rather than about the trial itself: the two funding applications, the
principal-investigator adoption guide, the large-document guidance, and the
patient advocacy paper \cite{fundapp1,fundapp2,piadoption,phase1guidance,%
patientadvocacy}. A request for \$1,606,000 is a request to run a trial, not to
write one. Figure 5 restates the whole register as four typed records, so the
encumbrance column can be read down rather than looked up.
...
\legkey{10.60}{-5.75}{protoblue}{primary key, an owned identifier}
\legkey{10.60}{-6.20}{pagrayd}{foreign key, a milestone this row blocks}
\node[font=\tiny\itshape,text=protogray,anchor=north west,text width=132mm]
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 5. The asset register as four typed records. Thirteen rows\\
...
The four legislative drafts in the register are a separate line of work and are
not on the trial's critical path: they supplied a verification vocabulary and a
catalogue of clinician objections, and nothing in the protocol, the IND, or this
plan is contingent on any of them being enacted
\cite{hr9510billv5,congressvote,clintrust,verifygenact}.
...
Seven instruments are absent. Table 7 names the milestone each one blocks and
the single party that can supply it, and no row on it can be closed by the
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 7. Seven absent instruments, the milestone each blocks, and the\\
single party that can supply it. The last column reads no on every\\
row, which is the whole content of the table and the reason \S8 exists.}
...
\node[umlpkgtab,anchor=south west] at ([yshift=0.6mm]bs.north west)
  {ChemicalQDevice, sponsor scope, 6 cases};
...
\node[umlpkgtab,anchor=south west] at ([yshift=0.6mm]bu.north west)
  {UC San Diego Moores, site scope, 6 cases};
...
\node[umlpkgtab,fill=pagraym,anchor=south west] at ([yshift=0.6mm]bb.north west)
  {Blocked, no instrument exists, 3 cases};
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 6. Two system boundaries, twelve use cases, and the corridor\\
...
Three use cases sit in the corridor between the two boundaries of Figure 6, and
each is blocked by exactly one signature. No amount of company work removes any of the three, which is the
