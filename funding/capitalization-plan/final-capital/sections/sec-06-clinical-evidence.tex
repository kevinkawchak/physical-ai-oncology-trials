...
% FINAL stage improvements: Figure 16's tier column is given a three-swatch
% legend so tier is readable without the table; Figure 17's four ranks are
% numbered, so a reader can follow one chain across the figure by number rather
% than by tracing an edge.
...
false description. Table 16 states all six with their comparator, their evidence
tier and the limitation their own authors gave them.
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 16. Six quantities with their comparator, their evidence tier, and\\
the limitation each set of authors stated. No row can be read without the\\
caveat that qualifies it, which is the condition on citing any of them.}

Figure 16 draws the same six as intervals a reviewer can measure, each beside
its comparator and the limitation that qualifies it.
...
\legkey{4.40}{0.66}{protowhite}{trial}
\legkey{6.20}{0.66}{pagrayl}{in silico or twin}
\legkey{9.20}{0.66}{protoblue}{peer-reviewed clinical readout}
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 16. Six quantities a reviewer can check against a published\\
...
Figure 17 traces five published quantities to the exact filing section each one
reaches, and labels every edge with the kind of claim it licenses.
...
\end{scope}
\node[gvctitle,anchor=south west] at ([yshift=1mm]cq.north west) {Rank 1, published quantity};
...
\end{scope}
\node[gvctitle,anchor=south west] at ([yshift=1mm]cn.north west) {Rank 2, IND section};
...
\end{scope}
\node[gvctitle,anchor=south west] at ([yshift=1mm]cr.north west) {Rank 3, protocol section};
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 17. Five published quantities and the exact filing section\\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 17. Three work products against their industry cost and\\
schedule benchmarks. The author figures are projected from the\\
source set and are reported as such, not as audited accounts.}
