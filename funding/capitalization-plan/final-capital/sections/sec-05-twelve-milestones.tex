...
% FINAL stage improvements: Figure 13 is the one figure in the paper whose
% optical centre sits left of its frame, because its left-hand lane titles are
% narrow and its right-hand cost column is wide.  It is carried in the author's
% \DiagramXShift adjustwidth wrapper, correction 11 of useredits.md, which
% nudges the frame without touching the caption spacing.  Figure 14 gains a
% gate-type legend; Figure 15 gains a branch owner on each column title.
...
\begin{appfloat}[!tb]
\begin{adjustwidth}{\DiagramXShift}{\dimexpr-\DiagramXShift\relax}
...
\end{adjustwidth}
\vspace{-0.65cm}
\figcaption{Thirty-three months, twelve milestones, and the artifact date on\\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{The twelve milestones with their months and cost. M1 to M5 sum to
\$306,000 and M6 to M12 sum to \$1,300,000; the same twelve costs appear as work
packages WP1 to WP12 in Figure 9.}
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{The same twelve rows keyed by ID, carrying what proves each milestone,
what halts it, and which mechanism pays. Every stop condition is a number or a
signed instrument; none is a judgement call.}
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{One milestone, three concurrent evidence branches, and the join\\
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{One top event, five halt points, and ten basic events below.\\
...
The two OR branches are the honest part of the figure. H1 fails on either a
