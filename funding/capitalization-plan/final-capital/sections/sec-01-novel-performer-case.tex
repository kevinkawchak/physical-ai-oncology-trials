...
% FINAL stage improvements: Figure 1 gains a survivor count and a legend row;
% Figure 2 gains a column-5 tally so the two well-suited rows can be counted
% without reading them; Figure 3 gains an explicit equals rule under the three
% totals.  A \clearpage before the last subsection clears the 143pt overfull
% vbox that stage 7 found on page 9.
...
deferring to ``the same incumbents that consume the funding''. It is the clause
the report supports with the person-based funding comparison and with the
productivity slowdown literature \cite{azoulay2011incentives,scannell2012eroom},
and the ten prior applications were written under it \cite{tenapps2026}.
...
Four eligibility tests separate the three, and only one clause passes all four.
Figure 1 runs the three candidates through the filter, and Table 4 states each
test against the sentence in the report it is drawn from.
...
\legkey{0}{-3.95}{pablue2}{clause under test}
\legkey{4.2}{-3.95}{pagrayl}{clause eliminated}
\legkey{8.4}{-3.95}{protoblue}{clause surviving, 1 of 3}
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 1. Three clauses of one report, four eligibility tests, and\\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 4. The four tests, the sentence in the report that each is\\
drawn from, and this company's answer. Clause 1 fails T1, clause 2\\
fails T2 and T4, and clause 3 is the only one that survives all four.}
...
report is written against. The same holds for the two executive orders that sit
behind the report: the Genesis Mission is a national compute and robotics
programme and the gold-standard science order binds all federally funded
research, and neither is a mechanism a firm applies to
\cite{eo14363genesismission,eo14303goldstandard}.
...
\node[d2cellk,minimum width=20mm,text width=17mm,minimum height=6.2mm,
      font=\tiny\sffamily\bfseries] at (12.95,-6.02) {2 well, 3 partly, 2 less};
\legkey{0.0}{-6.60}{protowhite}{Well or partly suited, the report's own scoring}
\legkey{6.6}{-6.60}{pagrayl}{Less suited}
\legkey{9.6}{-6.60}{protoblue}{Well suited, this company}
\node[font=\tiny\itshape,text=protogray,anchor=north west,text width=132mm]
  at (0,-7.00) {The rule at the right marks where the report's own columns end
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 2. The report's own seven activities, its four institutional\\
...
Two well suited, three partly suited, two less suited: that is the fifth column
Figure 2 adds to the report's own institutional-form table. The two well-suited
rows are public-goods data and infrastructure, and mission-driven public-good
...
with a fixed appropriation can read directly. Figure 3 prices the identical
direct work three ways, and Table 5 sets the same three routes out line by line.
...
\draw[protoblue,line width=0.6pt] (0,-3.10) -- (14.65,-3.10);
\node[font=\tiny\sffamily\bfseries,text=protoblue,anchor=north west] at (0,-3.16)
  {one direct base, three totals};
...
\begin{scope}[shift={(1.85,-4.35)}]
  \hbarrow{0}{3.20}{pagraym}{Route A}{\$2,137,000}
  \hbarrow{-0.55}{3.13}{pagraym}{Route B}{\$2,091,000}
  \hbarrow{-1.10}{2.41}{protoblue}{Route C}{\$1,606,000}
  \axisy{0.22}{-1.32}{0}
\end{scope}
\node[font=\tiny\sffamily,text=protogray,anchor=north west,text width=44mm]
  at (9.10,-3.95) {Bars share one scale: 3.2 cm is \$2,137,000. Route C is
  75.2 percent of route A for the identical direct work.};
\node[font=\tiny\itshape,text=protogray,anchor=north west,text width=132mm]
  at (0,-6.05) {Every column begins at the same \$1,396,000 and every total is
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 3. The same 1,396,000 of direct work priced three ways. A\\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 5. The same direct work under three overhead regimes. The 57 percent rate\\
