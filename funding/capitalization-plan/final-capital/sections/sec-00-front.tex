% sec-00: abstract, executive summary, reader's guide, and the three indexes.
% FINAL stage.  Senior-author pass: the abstract is tightened by one clause,
% every table and figure cross-reference in Table 1 is checked against the
% float that carries it, and the reader-guide column widths are re-cut against
% the compiled widths rather than the estimated ones.
\section*{Abstract}
\phantomsection\addcontentsline{toc}{section}{Abstract}

In August 2026 the author filed ten funding applications for a Phase 1 trial of
an LLM-advised robotic pancreaticoduodenectomy with perioperative daraxonrasib,
...
shortest of the ten. This paper converts the author from independent scientist
to small-business operator and rewrites that application as the document it
should have been.
...
\midrule
SBIR program director & \S3, Table 11 & \S5, Tables 14 and 15 & Does the award buy a defensible gate, and does the budget sum? \\
ARPA-H programme manager & \S5, Figure 13 & \S1, Figure 1 & Is there a milestone I can hold someone to, and a stop condition? \\
Private investor & \S4, Table 13 & \S2, Table 6 & What do I own, when may I own it, and what is already built? \\
UC San Diego intake & \S8, Figure 19 & \S7, Figure 18 & What is being asked of this institution, and by when? \\
\end{tabularx}
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 1. Four readers and the section each should open first.\\
No reader is asked to read the document front to back, and every\\
entry point is a table or a figure rather than a paragraph.}

Two indexes follow, and they are the document's other entry points. Table 2
lists the twenty figures with the platform each is drawn on and the one question
each answers, and Table 3 does the same for the twenty-one tables. A reader who
...
10 & 4 & D2 & \appfile{d2/} & Where does every dollar come from, and what separates the tiers? \\
11 & 4 & PlantUML & \appfile{plantuml/} & Which capital positions may be held while a participant is on study? \\
...
14 & 5 & PlantUML & \appfile{plantuml/} & What do three parties do at once to close one milestone? \\
15 & 5 & Graphviz & \appfile{graphviz/} & What has to fail, and in what combination, to stop the programme? \\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 2. The twenty figures, their section, their platform, the directory that\\
...
\begin{apptable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.7cm} >{\raggedright\arraybackslash}p{1.1cm} >{\raggedright\arraybackslash}p{4.9cm} >{\raggedright\arraybackslash}X}
...
12 & 4 & The \S54.2 and \S54.4 disclosure triggers & What must be disclosed, at what threshold, on which form? \\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 3. The twenty-one tables, their section, their contents, and the one\\
question each answers. Every table is set to the full body measure with\\
ragged-right fixed columns, so no cell shows a stretched interword gap.}
