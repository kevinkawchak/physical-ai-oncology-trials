...
% FINAL stage improvements: Figure 7's four guards are keyed G1 to G4 inside
% the gate state itself so the note beneath it is a legend rather than the only
% place the guards are named; Figure 8 gains a percentage column head on the
% ratio bars; Figure 9 gains a per-cluster package count.
...
confuses the two will not survive a first reading. Table 8 decomposes both
awards into the three, and the work figure inside them is \$1,396,000.
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 8. The two awards decomposed. A \$1,606,000 award carries \$1,396,000\\
of direct work at a 15.0 percent load, against an SBIR indirect\\
allowance of 40 percent of direct costs that this plan does not claim.}
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 7. One award in five states, and the four guards that must all\\
...
The four guards Figure 7 places on the month-nine gate are set out below, and
each carries the same key wherever it is named in this paper.
...
Table 9 carries the eight direct lines and the load that together make the
\$306,000.
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 9. Phase I in eight direct lines plus the load. No participant\\
is consented and no drug is dispensed inside Phase I: every\\
dollar buys either an instrument, a verification, or a signature.}

\subsection{Phase II, Twenty-Four Months, \$1,300,000}

Table 10 does the same for the twenty-four months, where six participants at
\$48,000 each is the largest single line.
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 10. Phase II in ten direct lines plus the load. Six participants at\\
\$48,000 each is the largest single line; if the site executes late, Phase II\\
is resubmitted for three participants, which is the Descoped state of Figure 7.}

\subsection{The Same Work, Priced Twice}

The same four layers carry two prices. Figure 8 draws the programme with the
SBIR route beside it and the delta as a third column, and Table 11 reconciles
the identical rows arithmetically.

\begin{appfloat}[!tb]
\begin{appfig}[d2-type / layered ledger]
% A three-column ledger read left to right, deliberately unlike Figure 10's
...
\begin{scope}[shift={(12.55,-3.15)}]
  ...
  \node[font=\tiny\sffamily\bfseries,text=protoblue,anchor=south west] at (0,3.15)
    {Share of the programme reached};
\end{scope}
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 8. One programme, four layers, and two prices for the same work.\\
...
\end{apptable}
\vspace{-0.65cm}
\tabcap{Table 11. The four-layer budget under two prices. Every row\\
reconciles: the middle column plus the right column equals the\\
left, and the three totals are the sums of their own columns.}
...
Figure 9 breaks the programme into seventeen work packages and every edge
between them, and marks which of the three funding levels reaches each.
...
\node[gvcellh,minimum width=20mm,text width=17mm,anchor=south east]
  at ([xshift=-1mm,yshift=1mm]cp1.north east) {5 WP, \$306,000};
...
\node[gvcellh,minimum width=20mm,text width=17mm,anchor=south east]
  at ([xshift=-1mm,yshift=1mm]cp2.north east) {7 WP, \$1,300,000};
...
\node[gvcellg,minimum width=20mm,text width=17mm,anchor=south east]
  at ([xshift=-1mm,yshift=1mm]cpg.north east) {5 WP, \$2,104,000};
\draw[gvedge] (w1) -- (w2);
...
\end{appfig}
\vspace{-0.65cm}
\figcaption{Figure 9. Seventeen work packages and every edge between them. Five\\
