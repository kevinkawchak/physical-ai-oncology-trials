...
Cross-Repository LaTeX Paper Chunking
v2.9.2 - Cross-Repository LaTeX Paper Chunking

## Summary

Chunked 2 large LaTeX paper files from external repositories (national-mcp-pai-oncology-trials and pai-oncology-trial-fl) into smaller files within the physical-ai-oncology-trials repository. Each source paper was split into 3 chunk files at logical section boundaries, with corresponding bibliography files and README.md reconstruction instructions. This avoids Claude Code Opus 4.6 errors caused by the 20,000 token-per-file limit.

## Features

- Chunked `National_MCP_Servers_for_Physical_AI_Oncology_Clinical_Trial_Systems.tex` (1,011 lines, ~62 KB) from national-mcp-pai-oncology-trials into 3 files in `national-platform/national_mcp/`
- Chunked `main.tex` (930 lines, ~57 KB) from pai-oncology-trial-fl into 3 files in `national-platform/federated_learning/`
- Each chunk directory contains a README.md with file descriptions, reconstruction commands, and context preservation notes
- Included corresponding `references.bib` files in each chunk directory
...
- Files are split at logical section boundaries (Introduction/Methods, Results, Discussion/Conclusion) to maintain context
- Concatenating chunk files in order reproduces the original file exactly
- Cross-references (\ref, \cite) span chunks; the references.bib file is required for bibliography resolution
