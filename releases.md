...
Five British spellings survive in the directory and all five are inside file paths: the capitalization plan contains a section file named sec-03-gate-and-programme.tex, and a source map that cites a real path cannot rename it. Every word of authored prose in the block is American English, and the dialect audit over a thirty-seven word list returns zero outside those paths and the verbatim prompt.
