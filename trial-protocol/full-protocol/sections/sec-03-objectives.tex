...
analysis plan in \S9 and the safety oversight rules in \S6 and \S7.
...
\addlinespace
Describe the fidelity of preoperative simulation to the operative event. & Sim-to-real gap in trajectory (millimeters) and tip force (newtons) between Phase 0 simulation and the intra-operative record. & The sim-to-real gap validates the Phase 0 simulation-validation gate and supports the digital-evidence basis for the device feasibility claim. \\
\bottomrule
\end{tabularx}

\vspace{0.3em}

\noindent Abbreviations: DLT, dose-limiting toxicity; ISGPS, International Study
...
first-in-human study is not powered for formal comparison against them.
...
\node[mmin]   (s1)   at (-4.2,-7.2){R0 rate; ISGPS\\B/C fistula};
\node[mmin]   (s2)   at (4.2,-7.2) {Clavien-Dindo III+;\\30/90-day mortality};
\node[mmgoal] (exp)  at (0,-9.6)   {EXPLORATORY\\non-confirmatory};
\node[mmin]   (e1)   at (0,-12.0)  {PFS/OS to 24 mo;\\LLM concordance;\\sim-to-real gap};
\draw[mmedge] (prim) -- (p1);
\draw[mmedge] (prim) -- (p2);
\draw[mmedge] (prim) -- (p3);
\draw[mmedge] (sec) -- (s1);
...
safety at the discretion of the supervising surgeon, in the absence of device
malfunction, does not count against this endpoint but is recorded and reported.
Task-completion without unsafe conversion is the early-feasibility readout
recognized by the FDA for a significant-risk device \cite{fda2013earlyfeasibility}.
