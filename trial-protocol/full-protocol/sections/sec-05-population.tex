...
\node[mmdec] (ex) at (4.6,0)    {Excluded:\\not meeting \S5.1 /\\meeting \S5.2;\\declined; unresectable\\on staging};
...
\node[mmstep](d3) at (4.6,-6.8) {DL3 cohort\\(target dose)\\n = 3-6};
\node[mmdec] (di) at (9.2,-6.8) {Discontinued\\intervention\\(conversion,\\withdrawal, \S7)};
...
\draw[mmedge] (en) -- (tr);
\draw[mmedge] (tr) -- (d3);
\draw[mmedged] (tr) -| node[below,font=\scriptsize,pos=0.25]{protocol-defined} (di);
\draw[mmedge] (tr) -- (an);
\end{mermaidfig}
...
pancreaticobiliary surgery and to receive daraxonrasib.
\item A measured baseline CA 19-9 value (no threshold is imposed; the value is
recorded for serial assessment).
\item Eligible for perioperative daraxonrasib under the broad pan-KRAS criteria of
the RASolute 302 program \cite{rasolute302}.
...
adequate for the assigned operative tasks.
\end{enumerate}
...
during the study where applicable.
