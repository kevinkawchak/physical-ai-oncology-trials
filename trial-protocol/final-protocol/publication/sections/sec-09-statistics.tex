...
in this section and in the Statistical Analysis Plan that this section
summarizes, and are reported in accordance with TRIPOD+AI statement for model-assisted advisory component \cite{Collins2024tripodai}.
...
(nominal $\alpha = 0.05$, two-sided), and no multiplicity adjustment is applied due to descriptive analyses and non-confirmatory.
...
efficacy hypothesis.}
\label{tab:opchar}
\begin{tabularx}{\textwidth}{L{2.6cm} L{3cm} Y}
\toprule
...
derivation of four populations and cohort-level interim stopping logic.

\begin{mermaidfig}
\node[mmin]  (enr) at (0.3,-1.15)      {Enrolled and\\dose-assigned\\n = 18};
\node[mmstep](saf) at (4.4,2.3)  {Safety population\\all who received\\any intervention};
\node[mmstep](dlt) at (4.4,0) {DLT-evaluable\\completed 28-day\\window or had DLT};
\node[mmstep](pp)  at (4.4,-2.3) {Per-protocol\\Whipple + advisory\\per protocol};
\node[mmstep](mitt)at (4.4,-4.6) {modified ITT\\dosed with $\geq 1$\\post-baseline assessment};
\node[mmdec] (stop)at (10.3,2.3)  {Interim safety\\review after each\\cohort + DSMB};
\node[mmgoal](halt)at (8,-0.6) {Halt: $\geq 33\%$\\device-related\\serious AE, or 2\\device-related deaths};
\node[mmgoal](cont)at (11.7,-0.215) {Continue\\escalation};
\draw[mmedge] (enr.north east) -- (saf.south west);
\draw[mmedge] (enr.north east) -- (dlt.west);
\draw[mmedge] (enr.south east) -- (pp.west);
\draw[mmedge] (enr.south east) -- (mitt.north west);
\draw[mmedge] (saf) -- (stop);
\draw[mmedge] (stop.south west) -- node[left,font=\scriptsize, yshift=0.2cm, xshift=0.1cm]{met} (halt.north);
\draw[mmedge] (stop.south east) -- node[left,font=\scriptsize,pos=0.6, yshift=0.1cm, xshift=-0.1cm]{below} (cont.north);
\end{mermaidfig}
\vspace{-0.6cm}
...
\label{tab:secondary}
\begin{tabularx}{\textwidth}{L{4.75cm} L{3.6cm} Y}
\toprule
