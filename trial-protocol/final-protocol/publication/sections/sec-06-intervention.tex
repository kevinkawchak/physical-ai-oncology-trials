...
surgical context and for the drug-device use here.
...
$\S$312.21(e). The advisory control loop and platform architecture are shown
...
arm's tool assignment, operative-phase coverage driving Schedule of
Activities.}
...
\begin{table}[t]
\caption{Per-arm tool assignment, operative-phase coverage for the eight-arm LLM-directed Whipple platform.}
...
generates the operative and pathology inputs to the restart advisory), with permitted autonomy levels, human-oversight requirements for each procedure.
...
\node[mmstep](r14) at (5.2,-5.0)  {Restart T+14d\\Grade B / delayed\\serum rise (3 of 32)};
\node[mmdark](r21) at (10.4,-5.0)  {Restart T+21d /\\hold; Grade C\\(0 of 32 in sweep)};
\draw[mmedge] (pre) -- (in1);
\draw[mmedge] (pre) -- (in2);
\draw[mmedge] (in1) -- (adv);
\draw[mmedge] (in2) -- (adv);
...
(3 of 32), and T+21d or continued hold for Grade C (0 of 32 in the sweep). Advisory is bound, human-checked - not autonomous.}
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 9. Three reconstructive anastomoses and their controller
...
gastrojejunostomy (0.40-0.80 N), each guarded by Arm 5 ring-tension monitor issuing a soft warning outside band.}
...
assay, which both confirms exposure and supplies trough value used by
...
compliance with the cyber-physical envelope is verifiable for each case; any
deviation is detectable and time-stamped. The audit trail also satisfies the -24 h to +72 h preservation requirement that applies around a reportable
...
system for the index operation. Strong inducers or inhibitors of the daraxonrasib principal
metabolic pathway are avoided where feasible; when an interacting
agent is clinically unavoidable, its use is documented and reviewed versus exposure data.
