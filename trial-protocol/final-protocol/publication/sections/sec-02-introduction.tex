...
daraxonrasib restart causes dehiscence or micrometastatic outgrowth - the
timed advisory prevents.}
...
\node[mmgoal](m2) at (5.3,-1.7)   {10 kHz heartbeat, $\leq 3$ ms\\e-stop, force caps};
\node[mmgoal](m3) at (5.3,-3.4)   {Human-in-the-loop; surgeon\\plus backup plus ISM};
\node[mmgoal](m4) at (5.3,-5.1)   {Cross-framework\\(Isaac, MuJoCo, $\geq 2$)};
\node[mmgoal](m5) at (5.3,-6.8)   {On-premises LLM,\\open weights, isolated};
\node[mmgoal](m6) at (5.3,-8.5)   {Domestic deployment;\\data residency preserved};
\node[mmgoal](m7) at (5.3,-10.2)  {Deny-by-default,\\5 MCP servers,\\simplified gate};
\node[mmgoal](m8) at (5.3,-11.9)  {Hash-chained audit\\trail; 21 CFR part 11 traceable};
\draw[mmedge] (c1) -- (m1);
\draw[mmedge] (c2) -- (m2);
\draw[mmedge] (c3) -- (m3);
\draw[mmedge] (c4) -- (m4);
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 4. The mapping of all eight Physical AI concerns to protocol mitigations, the same eight pairs in Table 1. Each
concern is paired with a concrete, prespecified safeguard: validated device behavior over open-endedness, layered hard safety limits, retained human roles, and
multi-framework non-single-source validation.}

\vspace{0.3em}

\noindent\begin{tabularx}{\textwidth}{L{5.5cm} Y}
\toprule
\textbf{Concern} & \textbf{Protocol mitigation} \\
\midrule
Physical AI limitations & Phase 0 simulation validation over $\geq 1000$ procedures and $\geq 2$ independent frameworks; device behavior is narrow and locked, not generative, and operates only within validated envelopes. \\
...
\node[mmstep](b3)  at (4.9,-6.4)  {Optimized restart timing};
