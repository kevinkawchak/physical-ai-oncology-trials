...
\begin{mermaidfig}
\node[mmin]  (st1) at (0,0)     {Stage 1\\Clinician-controlled\\teleoperation /\\supervised;\\full autonomy\\prohibited \S312.21(e)};
\node[mmdec] (r1)  at (4.6,0)   {Independent safety\\review + USL\\reassessment};
\node[mmstep](st2) at (9.2,0)   {Stage 2\\Supervised\\semiautonomous;\\Class II, continuous\\oversight, immediate\\manual override};
\node[mmdec] (r2)  at (9.2,-3.6){Accumulated safety\\data + DSMB\\concurrence};
\node[mmgoal](st3) at (4.6,-3.6){Stage 3\\Higher autonomy\\after review\\(reserved,\\later phase)};
\draw[mmedge] (st1) -- (r1);
\draw[mmedge] (r1) -- (st2);
\draw[mmedge] (st2) -- (r2);
\draw[mmedge] (r2) -- (st3);
...
review, a USL reassessment, and DSMB concurrence, reserved for a later phase.}
...
\node[mmdec] (q1)  at (4.2,0)    {DLTs in first 3?\\(28-day window)};
\node[mmstep](dl1e)at (4.2,-2.6) {Expand DL1\\to 6};
\node[mmdec] (q1b) at (4.2,-5.2) {DLTs in 6?};
\node[mmstep](dl2) at (8.4,0)    {DL2 220 mg QD\\escalate, enroll 3};
\node[mmgoal](dl3) at (8.4,-2.6) {DL3 300 mg QD\\target dose,\\enroll 3-6};
\node[mmgoal](mtd) at (8.4,-5.2) {Declare MTD / RP2D\\at prior dose level};
\draw[mmedge] (dl1) -- (q1);
\draw[mmedge] (q1) -- node[above,font=\scriptsize]{0 of 3} (dl2);
\draw[mmedge] (q1) -- node[right,font=\scriptsize]{1 of 3} (dl1e);
\draw[mmedge] (dl1e) -- (q1b);
\draw[mmedge] (q1b) -- node[below,font=\scriptsize]{1 of 6} (mtd);
\draw[mmedge] (q1b) -| node[below,font=\scriptsize,pos=0.25]{$\geq 2$ of 6} (mtd);
\draw[mmedge] (dl2) -- (dl3);
\draw[mmedge] (dl3) -- node[right,font=\scriptsize]{tolerated} (mtd);
\end{mermaidfig}
\figcaption{Figure 5. Daraxonrasib 3+3 dose-escalation scheme. Cohorts of three
advance when no dose-limiting toxicity is observed in the 28-day window, expand to
six on a single dose-limiting toxicity, and define the maximum tolerated dose and
