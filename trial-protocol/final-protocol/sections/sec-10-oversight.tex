...
\node[mmin]  (cro) at (0,-4.8)    {CRO / clinical\\monitor (robotics\\+ AI competent)};
\node[mmstep](ism) at (8.4,-4.8)  {Independent Safety\\Monitor (per\\procedure)};
\node[mmin]  (site)at (4.2,-7.0)  {Site team: surgeon,\\operator, backup\\operator};
\draw[mmedge] (si) -- (irb);
\draw[mmedge] (si) -- (dsmb);
\draw[mmedge] (si) -- (pais);
...
\draw[mmedged] (pais.south) -- node[right,font=\scriptsize,pos=0.6]{USL changes} (ism.north);
...
Physical AI Safety Review Committee on a $\leq 90$-day cadence, designates an
Independent Safety Monitor for each procedure, and delegates monitoring to a
...
\begin{mermaidfig}
\node[mmin]  (intro)at (0,0)      {Investigator presents\\study: disease,\\alternatives, prognosis};
\node[mmstep](pai)  at (4.4,0)    {Explain Physical AI\\system: role, oversight,\\risks (\S312.60(f))};
\node[mmdec] (opt)  at (8.6,0)    {Participant\\choice};
\node[mmgoal](yes)  at (8.6,-2.6) {Consents to Physical AI\\signed ICF + assent\\if applicable};
...
\node[mmin]  (wd)   at (0,-4.9)   {Right to withdraw at\\any time without penalty};
\draw[mmedge] (intro) -- (pai);
\draw[mmedge] (pai) -- (opt);
\draw[mmedge] (opt) -- node[right,font=\scriptsize]{accept} (yes);
\draw[mmedge] (opt.west) -- node[above,font=\scriptsize]{decline AI} (no.east);
...
with the standard right to withdraw at any time without penalty.}
...
that no autonomy-bearing change, and no escalation decision, proceeds without the
...
$\S$312.57 (at least two years after a marketing application is approved for the
...
preserved as described in \S10.4 so that any record can be reconstructed on FDA
request.
