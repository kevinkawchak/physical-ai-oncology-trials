...
small and the accountability chain is short, which is appropriate level of
...
\end{table}
\vspace*{-0.6cm}
\figcaption{Table 10.1. AEach of the four
...
assent provisions are inapplicable in practice even though informed-consent architecture (21 CFR part 50 with ICH E6(R3)) provides for assent
...
target indication does not arise materially in the pediatric population. 
...
\end{table}
\vspace*{-0.75cm}
\figcaption{Table 10.2. Oversight bodies, their cadence, and their authority for
...
\begin{mermaidfig}
\node[mmgoal] (si)    at (4.6,1)      {Sponsor-Investigator\\K. Kawchak\\COI disclosed, 21 CFR part 54};
\node[mmproc] (irb)   at (-1,-3.0)  {Reviewing IRB\\(human-subjects protection)};
\node[mmproc] (dsmb)  at (4.6,-3.0)   {DSMB\\cohort-boundary + triggered;\\halt authority};
\node[mmproc] (ism)   at (10.2,-3.0)   {Independent Safety Monitor\\per-procedure, real-time;\\stop authority};
\node[mmproc] (pais)  at (-1,-5)  {Physical AI Safety Review\\Committee ($\leq$ 90-day cadence)};
...
\draw[mmedge] (si.south) to (irb.north);
\draw[mmedge] (si) -- (dsmb);
\draw[mmedge] (si.south) to (ism.north);
\draw[mmedge] (si.south) to (pais.north east);
\draw[mmedge] (dsmb) -- (halt);
\draw[mmedge] (halt) -- (dec);
\draw[mmedge] (ism.south) to (dec.north east);
\draw[mmedge] (pais.south) to (dec.north west);
\end{mermaidfig}
\vspace*{-0.5cm}
\figcaption{Figure 20. The Data Safety Monitoring Board (cohort-boundary and triggered reviews,
...
less). The binding halt rules, a device-related serious adverse event in $\geq$
...
with 21 CFR \S 312.404. 

The vascular no-fly gate samples at 10 kHz and enforces a
...
millisecond time scales and are not discretionary;\\the DSMB binding halt rules act
...
Physical AI AE under 21 CFR part 11.
...
of the advisory oracle away from its validated behavior. 

A sim-to-real divergence
...
\end{table}
\vspace*{-0.55cm}
\figcaption{Table 10.4. Each trigger carries a defined reporting clock, and each generates an
...
\end{table}
\vspace*{-0.55cm}
\figcaption{Table 10.5. Seven Physical AI record types, their content, and
regulatory anchors. Record type seven binds the preceding six to a deterministic
seed and repository commit so reported behavior can be replayed
deterministically.}
