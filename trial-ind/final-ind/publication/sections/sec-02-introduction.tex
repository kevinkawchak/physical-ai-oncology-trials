...
resection while layering hard cyber-physical safety limits and optimizing
perioperative drug-restart.
...
Dutch nationwide cohort of 1000 robotic pancreaticoduodenectomies anchors
benchmark against which secondary feasibility and safety endpoints of this
Phase 1 investigation are interpreted descriptively, not as a powered comparator.}
\label{tab:sec02-baseline}
\centering
\begin{tabularx}{\textwidth}{L{2.95cm} C{2.4cm} Y}
\toprule
\textbf{Outcome} & \textbf{Dutch 2025 cohort} & \textbf{Relevance to this investigation} \\
\midrule
Unplanned conversion to open & 10.1\% & Anchors technical-feasibility co-primary endpoint (finished without unplanned device attributable conversion) \\
...
30-day mortality & 3.9\% & Anchors 30-day and 90-day mortality secondary endpoints \\
\bottomrule
\end{tabularx}
\end{table}
\vspace*{-1.4cm}
...
full autonomy is prohibited consistent with 21 CFR \S 312.21(e). 
...
artery, and a hard-stop emergency-stop zone at 5.0 mm. 
...
graded emergency-stop response times, and\\the distance-keyed vascular no-fly gate
...
These intervals are bounded by regulation and by the deliberate
...
\node[mmin]    (b1)  at (0.3,0)       {IND dossier\\(1571 to 11)};
\node[mmin]    (b2)  at (0.3,-1.9)    {Investigator's\\Brochure};
\node[mmin]    (b3)  at (0.3,-3.8)    {Clinical protocol};
\node[mmin]    (b4)  at (0.3,-5.7)    {Informed consent\\(ICF)};
\node[mmproc]  (d1)  at (10.5,0)    {SAE / SUSAR\\7 / 15-day};
\node[mmproc]  (d2)  at (10.5,-1.9) {DSUR (annual)};
\node[mmproc]  (d3)  at (10.5,-3.8) {Protocol\\amendments};
\node[mmproc]  (d4)  at (10.5,-5.7) {Dose-escalation\\minutes};
\node[mmgoal]  (a1)  at (5.4,-6.8)  {Tables, listings,\\figures};
\node[mmgoal]  (a2)  at (5.4,-8.4)  {CSR (ICH E3)};
\node[mmgoal]  (a3)  at (5.4,-10) {Manuscripts /\\lay summaries};
...
criteria, and the endpoint definitions are identical wherever they appear. 
...
\node[mmctx]  (route) at (9.4,0.395)  {Route: oral, once daily\\DL1 160 / DL2 220 /\\DL3 300 mg};
\node[mmdark] (btd)   at (9.4,-2.05)  {FDA Breakthrough Therapy\\(June 2025), pretreated\\metastatic KRAS G12 PDAC};
\draw[mmedge] (drx) -- (cypa);
\draw[mmedge] (cypa) -- (rason);
\draw[mmedge] (kras) -- (rason);
\draw[mmedge] (rason) -- (eff);
\draw[mmedge] (eff) -- (prol);
\draw[mmedged] (drx.north east) to (route.north west);
\draw[mmedged] (drx.south east) to (btd.north west);
\end{mermaidfig}
\vspace*{-0.5cm} 
\figcaption{Figure 5. Daraxonrasib forms a tri-complex with cyclophilin A and
...
anastomotic healing if restart is mistimed, which is why restart day is
...
escalation, and the wound-healing concern motivates the\\perioperative pause and restart advisory that keys the restart day to fistula grade and drug
trough.}
...
DL3 & 300 mg QD & 28 days & 3, expand to 6 if 1 DLT & RASolute 302 rec. Phase 2 dose; highest planned level \\
\bottomrule
\end{tabularx}
\end{table}
\vspace*{-1.4cm}
\figcaption{Table 3.4. Daraxonrasib oral dose levels and the 3+3 escalation
structure. At each level\\three participants are observed across the 28-day DLT
...
human experience: the drug exposure stays within the prior human range while
...
and supplies the operative and pathology inputs that restart advisory consumes
\cite{daraxwhipple}.
...
iterations (90.6 percent.

Grade A fistula with sub-threshold trough), T+14d in 3 of
...
malfunction or unsafe behavior. 

Binding halt rules govern progression: a
...
(Breakthrough Therapy) of unmet need in the same molecular population. Perioperative pause-and-restart logic additionally respects RASolve 301
...
with respect to combination and surgical setting, while the drug exposure
...
cohort-review packages that establish the recommended Phase 2 dose, the amendments
with synchronized consent, and the Phase 1-to-2 clinical study report and
end-of-Phase-2 briefing,\\to the Phase 2 randomized protocol at the 300 mg
...
which every downstream document inherits same pinned trial facts.
...
Physical AI System Description are designed to satisfy is summarized in Table 3.9. 
...
\centering
\begin{tabularx}{\textwidth}{L{5cm} L{3.0cm} Y}
\toprule
...
and LLM-evaluation evidence grounds the device,\\the integrated combination, and the
...
portability in mind, while regulatory status today remains investigational in
every jurisdiction with no marketing authorization.
...
before any patient-facing activity.

The supporting computational evidence base
...
integrated toxicology summary, and full data tabulation, is presented in \S 8.
...
authoring workflow that assembles the dossier in which this gate and rest of
