...
explicit guidance for the investigator. 

The full, standalone Investigator's
...
values appear once in brochure and are carried forward unchanged into every
module of IND \cite{daraxwhipple,onpremwhippl}.
...
generate toxicology or stability data that do not exist; brochure is one
indispensable component of that package and is reference safety text against
...
\node[mmin]   (ib)   at (5.2,2.6)     {Investigator's\\Brochure};
\node[mmin]   (nc)   at (11.3,-2.6)     {Nonclinical, CMC,\\stability};
\node[mmin]   (icf)  at (-1.2,-2.6)   {ICF + recruitment\\(Physical AI opt-out)};
\node[mmin]   (form) at (2.8,-2.6)    {Investigator forms\\1571 / 1572 / 3674};
\node[mmin]   (safe) at (7.4,-2.6)    {Safety\\information};
\node[mmproc] (clk)  at (5.2,-5)    {30-day FDA clock\\starts at submission};
\node[mmaccent] (acc) at (1.4,-7.4)   {Faster assembly starts\\the clock and all\\sites sooner};
\node[mmdec]  (lim)  at (9.0,-7.4)    {Cannot shorten 30 days\\or create missing\\toxicology / stability};
\draw[mmedge] (p)   -- (pkg);
\draw[mmedge] (ib)  -- (pkg);
\draw[mmedge] (nc)  to[out=-90,in=40,looseness=0.5] (clk.east);
\draw[mmedge] (icf) -- (pkg);
\draw[mmedge] (form) to[out=-90,in=140,looseness=0.5] (clk.west);
\draw[mmedge] (safe) to[out=-90,in=60,looseness=0.5] (clk.east);
\draw[mmedge] (pkg) -- (clk);
\draw[mmedge] (clk) -- (acc);
\draw[mmedge] (clk) -- (lim);
\end{mermaidfig}
...
data, the informed consent and recruitment materials,\\the investigator forms (1571,
...
\begin{table}[t]
\caption{Daraxonrasib (RMC-6236) physical, chemical, and pharmaceutical properties material to investigator.}
...
\begin{table}[t]
\caption{32-iteration deterministic perioperative pharmacokinetic sweep and resulting restart-day distribution.}
...
Restart day T+21 & 0 of 32 & 0\% & Reserved for slower trajectories; with a Grade C fistula maps to a continued hold and stopping rule \\
\bottomrule
\end{tabularx}
\vspace{-0.2cm}
\figcaption{Table 5.2. The baseline serum concentration of
0.45 ng/mL falls below the 0.5 ng/mL\\trough threshold in all 32 iterations, and the
...
\node[mmproc] (ft)    at (10.2,-2.6)  {8.1.3 Toxicology:\\full data tabulation};
...
\node[mmin]   (safem) at (5.2,-5.4)   {Safety margins; DL1\\160 mg conservative\\first-in-human};
\node[mmin]   (sweep) at (10.2,-5.4)  {32-iteration\\perioperative PK sweep\\(0.45 ng/mL baseline)};
\draw[mmedge] (pt) -- (ph);
\draw[mmedge] (pt) -- (tx);
\draw[mmedge] (pt) -- (ft);
\draw[mmedge] (ph) -- (pd.north);
\draw[mmedge] (ph) to (dsim.north);
\draw[mmedge] (tx) -- (safem);
\draw[mmedge] (ft) -- (sweep);
\end{mermaidfig}
\vspace*{-0.65cm}
\figcaption{Figure 13. The integrated toxicology summary states the safety margins
...
digital-twin and QSP simulation evidence and the
32-iteration perioperative pharmacokinetic sweep.}
...
device supplement and Physical AI System Description; controlling
hardware, software, and safety-system text is in \S 6 and \S 7 of this IND and in bound IB.}
\end{table}
...
\centering
\begin{tabularx}{\textwidth}{L{2.0cm} L{7.6cm} Y}
\toprule
\textbf{Phase} & \textbf{Activity and arm assignment} & \textbf{Closed-loop control} \\
\midrule
P1 to P4 & Dissection and specimen removal: Arms 1 and 2 dissect under Arm 3 vessel control (bipolar plus retractor) and Arm 4 near-infrared ICG imaging; Arm 7 suction and irrigation throughout & Five-vessel no-fly gate active at 10 kHz; force caps enforced; specimen removed at end of P4 \\
...
Heartbeat and watchdog & 10 kHz bus, 100 \textmu s deadline & On-time frame continues command state \\
Arm park & 50 microseconds maximum & Triggered by missed/out-of-parameter frame \\
...
supplement for expectedness. 

The perioperative pause-and-restart advisory is
...
\midrule
1 & Serious Physical AI adverse event & 7 days (unexpected fatal or life-threatening) or 15 days (other serious and unexpected), as for a drug suspected AE \\
...
expectedness reference against which each trigger is judged; audit trail is
preserved from 24 hours before to 72 hours after AEs.}
\end{table}
...
that reference text against which every expectedness determination is made
remains current throughout conduct of study
