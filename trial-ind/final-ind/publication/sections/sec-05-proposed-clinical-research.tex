...
early-feasibility pathway recognized for significant-risk devices, which generates device-performance and safety evidence required before any
...
attributable to device malfunction or unsafe device behavior. 
...
Co-primary 2: MTD and RP2D & Maximum tolerated dose and recommended Phase 2 dose by the 3+3 rule across the 28-day DLT window & DLT-evaluable & Dose level; DLT rate per level, exact 95\% CI \\
...
Exploratory: sim-to-real gap & Trajectory gap (mm) and tip-force gap (N) between simulation, operative record & Safety (all dosed) & Median (mm; N) \\
\bottomrule
\end{tabularx}
\end{table}
...
\label{tab:sec06-soa}
\centering
\begin{tabularx}{\textwidth}{L{5.5cm} C{1.4cm} C{1.6cm} C{1.6cm} Y}
\toprule
...
\begin{table}[t]
\caption{Table 6.3. 3+3 dose-escalation decision rule applied at
daraxonrasib dosages (DL1 160 mg, DL2 220 mg, DL3 300 mg once daily) across
28-day dose-limiting toxicity window. Max tolerated dose is highest
level at which fewer than two of six dose-limiting-toxicity-evaluable participants
experience a dose limiting toxicity.}
\label{tab:sec06-33}
\centering
\begin{tabularx}{\textwidth}{L{2.6cm} L{3.9cm} Y}
\toprule
\textbf{DLTs in first 3} & \textbf{Action} & \textbf{Resolution} \\
\midrule
0 of 3 & Escalate to next dose level & Current level cleared; move up one level (or declare RP2D at DL3) \\
1 of 3 & Expand current level to 6 & Enroll 3 more at the same level and re-evaluate at 6 \\
2 or 3 of 3 & Stop escalation at level & MTD exceeded; MTD is the preceding level \\
1 of 6 (after expansion) & Escalate to next dose level & Level tolerated; move up one level (or declare RP2D at DL3) \\
2 or more of 6 & Stop escalation at level & MTD exceeded; MTD is the preceding level \\
\bottomrule
\end{tabularx}
\end{table}
...
\label{tab:sec06-cp}
\centering
\begin{tabularx}{\textwidth}{C{1.6cm} C{2.35cm} C{2.6cm} Y}
\toprule
...
Table~\ref{tab:sec06-phase0} states the three Phase 0 gate conditions, numeric
...
\centering
\begin{tabularx}{\textwidth}{L{3.2cm} L{6.2cm} Y}
\toprule
\textbf{Condition} & \textbf{Threshold} & \textbf{Consequence if not met} \\
\midrule
Unified Safety Level (USL) rating & $\geq$ 7.0 on the surgical-readiness scale & No patient-facing robotic activity; remediation and re-rating required \\
Simulated procedure volume & $\geq$ 1000 simulated procedures across at least 2 independent frameworks (Isaac, MuJoCo, Gazebo, PyBullet) & Readiness finding not established; additional simulation required before any case \\
...
Tumor and vascular anatomy compatible with eight arm robot on preoperative imaging & Anatomy or comorbidity otherwise incompatible with safe conduct of the combined intervention \\
\bottomrule
\end{tabularx}
\end{table}
...
\centering
\begin{tabularx}{\textwidth}{L{4.6cm} L{3.9cm} Y}
\toprule
...
\needspace{8\baselineskip}
\begin{mermaidfig}
\node[mmin]  (pause) at (0.4,0)        {Daraxonrasib paused\\before surgery};
\node[mmin]  (in1)   at (0.4,-2.0)     {Input 1: PJ ISGPS\\fistula grade A / B / C};
\node[mmin]  (in2)   at (0.4,-4.0)     {Input 2: serum trough\\vs 0.5 ng/mL threshold};
\node[mmproc] (llm)  at (5.6,-2.0)   {LLM advisory\\(hash-pinned)\\+ human confirmation\\(never autonomous)};
\node[mmgoal] (t7)   at (10.8,0)     {Restart T+7d\\29 of 32 (Grade A,\\sub-threshold trough)};
\node[mmstep] (t14)  at (10.8,-2.0)  {Restart T+14d\\3 of 32 (Grade B or\\delayed serum rise)};
\node[mmdark] (t21)  at (10.8,-4.0)  {Restart T+21d or hold\\0 of 32 (Grade C)};
...
\end{mermaidfig}
\vspace*{-0.46cm}
\figcaption{Figure 15. The drug is paused before surgery and the postoperative restart day is set by a
hash-pinned, human-confirmed LLM advisory from two inputs: the
...
\centering
\begin{tabularx}{\textwidth}{L{2.4cm} L{4.2cm} Y C{2.0cm}}
\toprule
...
\centering
\begin{tabularx}{\textwidth}{L{4.6cm} L{5.6cm} Y}
\toprule
...
5. Sim-to-real divergence beyond $2\times$ tolerance & 15 calendar days; trajectory over 4 mm or force > 1.0 N & Audit trail $-24$h to $+72$h \\
...
\needspace{8\baselineskip}
\begin{mermaidfig}
\node[mmin]  (ev)  at (0,-3.8)      {Adverse event\\(clinical or\\Physical AI)};
...
\node[mmproc] (g3) at (9.7,-4.4)   {3 Cybersecurity incident (15d; 7d if harm)};
\node[mmproc] (g4) at (9.7,-5.8)   {4 Model degradation ($\geq 3$ procedures / 24h)};
\node[mmproc] (g5) at (5.4,-7.2)    {5 Sim-to-real divergence $> 2\times$ ($>4$ mm / $>1.0$ N)};
\node[mmproc] (g6) at (9.7,-7.2)   {6 Digital-twin discrepancy};
\node[mmdark] (aud) at (9.7,-2.4)  {Hash-chained audit trail\\$-24$h to $+72$h\\(21 CFR part 11)};
\draw[mmedge] (ev.north east) to[out=60,in=180,looseness=0.6] (c7.west);
\draw[mmedge] (ev.north east) to[out=60,in=180,looseness=0.6] (c15.west);
\draw[mmedge] (ev.south east) to[out=-80,in=180,looseness=0.5] (g1.west);
\draw[mmedge] (ev.south east) to[out=-90,in=180,looseness=0.5] (g2.west);
\draw[mmedge] (ev.south east) to[out=-95,in=180,looseness=0.5] (g5.west);
\draw[mmedge] (g3) -- (aud.south);
\draw[mmedge] (g4.west) to[out=150,in=180,looseness=0.5] (aud.south west);
\draw[mmedge] (g6.west) to[out=150,in=-180,looseness=0.5] (aud.south west);
\end{mermaidfig}
\vspace*{-0.6cm}
\figcaption{Figure 16. A fatal or life-threatening suspected adverse reaction is reported within
...
(trajectory over 4 mm or force over 1.0 N), and a digital-twin discrepancy.}
...
\centering
\begin{tabularx}{\textwidth}{L{3.3cm} Y L{4.6cm}}
\toprule
...
Device (index procedure) & Any hard-stop cross-arm E-stop event & Halt; resume after cause identified, safety re-passed \\
Drug (participant) & Drug-attributable DLT in the 28-day window; other unacceptable toxicity; Grade C fistula; pregnancy; illness; participant request & Discontinue daraxonrasib; continue safety and survival follow-up \\
...
none is filed, two years after investigation is discontinued and FDA is
notified.}
...
\centering
\begin{tabularx}{\textwidth}{L{4.4cm} Y L{3.3cm}}
\toprule
...
Physical AI System Description is specific to this trial.}
