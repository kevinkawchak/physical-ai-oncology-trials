...
\node[mmproc]   (f3)   at (4.0,-4.9)  {FDA Form 3674\\clinicaltrials.gov\\certification of compliance};
\node[mmctx]    (ba)   at (0.0,-7.8)  {Box A\\no clinical\\investigation};
\node[mmdec]    (bb)   at (4.4,-8.1)  {Box B\\402(j) PHS Act\\does NOT apply\\(typical Phase 1)};
\node[mmctx]    (bc)   at (8.6,-7.8)  {Box C\\402(j) applies;\\will comply};
\node[mmgoal]   (sub)  at (-1.8,-10.8) {Assembled IND\\submitted to FDA;\\30-day review\\clock starts};
\draw[mmedge] (cl) -- (f1);
\draw[mmedge] (f1) to[out=200,in=70,looseness=0.6] (ser.north);
\draw[mmedge] (f1) to[out=340,in=110,looseness=0.6] (f3.north);
\draw[mmedge] (f3) to[out=250,in=70,looseness=0.55] (ba.north);
\draw[mmedge] (f3) -- (bb.north);
\draw[mmedge] (f3) to[out=300,in=110,looseness=0.55] (bc.north);
\draw[mmedge] (bb) to[out=250,in=20,looseness=0.6] node[mmlabel]{selected for this trial} (sub.east);
\draw[mmedge] (ser) to[out=270,in=160,looseness=0.6] (sub.west);
\end{mermaidfig}
...
\begin{center}
\begin{xltabular}{\textwidth}{>{\raggedright\arraybackslash}p{3.0cm} Y >{\raggedright\arraybackslash}p{3.6cm}}
\toprule
...
their codified anchors in 21 CFR, and the manner in which each is
...
\needspace{12\baselineskip}
\begin{center}
\begin{xltabular}{\textwidth}{>{\raggedright\arraybackslash}p{1.5cm} Y >{\raggedright\arraybackslash}p{4.2cm}}
\toprule
...
\needspace{12\baselineskip}
\begin{center}
\begin{xltabular}{\textwidth}{>{\raggedright\arraybackslash}p{4.4cm} >{\raggedright\arraybackslash}p{2.6cm} Y}
\toprule
...
first dose \cite{congress9510, clintrustpai, paipredict4x}. Registering
...
daraxonrasib via the 3+3 design, and technical feasibility), so that the
trial's pre-specified stopping logic cannot be revised silently after
results emerge \cite{paiautospons, oncotrialllm}.
...
to a mandatory Box C certification without a gap in the public record.}
