...
appropriate to later phases \cite{phase1guidance}. The graded approach embodied
...
\node[mmproc] (ea)  at (14.4,0)     {7.2 Environmental\\assessment /\\categorical\\exclusion 25.31};
\node[mmin]   (ds1) at (0,-5.6)     {Characterization,\\specification,\\impurities};
\node[mmin]   (dp1) at (4.2,-5.6)   {Composition,\\container-closure,\\child-resistant};
\node[mmin]   (st)  at (8.4,-5.6)   {Stability program\\(supports shelf life)};
\draw[mmedge] (cmc) -- (ds);
\draw[mmedge] (cmc) -- (dp);
\draw[mmedge] (cmc) -- (pl);
\draw[mmedge] (cmc) -- (lb);
\draw[mmedge] (cmc.east) to[out=0,in=120,looseness=0.6] (ea.north);
\draw[mmedge] (ds) -- (ds1);
\draw[mmedge] (dp) -- (dp1);
\draw[mmedge] (dp) to[out=-90,in=120,looseness=0.6] (st.north);
\end{mermaidfig}
\figcaption{Figure 17. The Chemistry, Manufacturing and Control information
...
acceptance criteria are scaled to the early phase of investigation and will be
tightened as batch and process knowledge accumulate.}
\end{table}
...
in-process parameters and as release tests. The regulatory starting materials are
...
conforms; non-conforming material is not released for use in clinical drug-product
manufacture.
...
form respectively. At the Phase 1 stage these procedures are qualified rather than
...
\begin{tabularx}{\textwidth}{L{2.4cm} C{2.4cm} L{3.4cm} Y}
\toprule
...
common-blend design and the physical differentiation among strengths support
...
\bottomrule
\end{tabularx}
\figcaption{Table 7.8. Interim Phase 1 drug-product specification. The dissolution
...
retest period. The accelerated arm at 40 degrees Celsius and 75 percent relative
...
$\S$7.1.4. Because the perioperative regimen pauses dosing before surgery and
...
\bottomrule
\end{tabularx}
\figcaption{Table 7.10. Drug-product stability program. Stability-indicating
...
in $\S$6.1 of this IND rather than through a placebo. These outcome-assessor
...
masking, would be submitted by information amendment under 21 CFR \S 312.31 at
that time.
...
$\S$312.31 before the affected material is administered, so that the CMC
...
drug-accountability record, so that no meaningful quantity of the drug substance
is released to the general environment. The durable robotic device and its
