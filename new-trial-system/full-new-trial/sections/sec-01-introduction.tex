% sec-01-introduction.tex - Section 1, Introduction.  FULL stage.
% Figures 1 and 2 drawn; Tables 4 and 5 populated.  Prose held to one page.
...
\begin{apptable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{5.25cm} >{\raggedright\arraybackslash}p{2.05cm} >{\raggedright\arraybackslash}X}
...
evidence for that sentence: \S2 gives the method, \S3 through \S7 give the four
deliverables and the review regime that cleared them, and \S8 states what
remains exposed.
...
\end{appfig}
\vspace{-0.6cm}
\figcaption{Figure 2. Ten operating axes on which the prior and new trial systems
differ,\\each cell carrying a measured value from the author's deposited record.}
\end{appfloat}
...
\midrule
Document production unit & Team-month & Prompt-hour & \S2 \\
...
\S2 gives the method and the two properties that make it repeatable. \S3 through
\S6 give the four deliverables the method produced, each written to a similar
length because each is load-bearing: the IND, the two trial protocols, the
legislative framework, and the funding proposals. \S7 gives the review regime
that cleared all four and the measurements that justify trusting it. \S8 states
what remains exposed and what future work consists of, and \S9 states what
