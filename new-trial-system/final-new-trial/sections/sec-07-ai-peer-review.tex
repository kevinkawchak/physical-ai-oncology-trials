...
% FINAL stage.  Figures 21, 22, 23 and 24 drawn; Tables 21 to 24 populated.
% Primary source is the AI peer review archive under new-trial-system/inputs;
% the role assignment comes from funding/RFA-RM-27-001-v2.  Senior-author pass:
% Table 21's Prior column loses 0.05 cm to its Ratio column, and two closing
% subsections give the chronology by which reviewer competence was established
% and name the four things a human still does.
...
modification to the pancreatic cancer work reported in \S\ref{sec:ind} through \S\ref{sec:funding}. The
...
\begin{apptable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.85cm} >{\raggedright\arraybackslash}p{3.50cm} >{\raggedright\arraybackslash}p{3.35cm} >{\raggedright\arraybackslash}X}
...
Nothing in \S\ref{sec:peerreview} substitutes for an institutional review board, an FDA
...
before they were relied upon, and the record of that showing runs from late 2025
to the present. In October 2025 an end-to-end oncology trial efficiency study
...
\cite{aiprgliomatc}. In December 2025 a code generation competition ran sixteen
proprietary and open-source models against FDA adverse event data with iterative
learning, which is a direct measurement of reviewer competence rather than an
assumption of it \cite{codegencomp2025}. By the time the pancreatic cancer work
of \S\ref{sec:ind} through \S\ref{sec:funding} began in 2026, the question was
no longer whether the models could review; it was which model should hold which
...
reliable in general. It is that these models, in these roles, on this class of
artifact, produced measurable corrections on five occasions that are separately
deposited and separately dated, and that the corrections are visible in the
diffs. A reviewer who doubts the claim can read the corrections rather than the
argument.
...
investigator, and two of its five dispositions stop the artifact rather than
passing it. And the person signs: every deposit, every disclosure, and every
statement of what the work does not claim is the author's, and the funding
application says so in the plainest terms available, that the models ``are not
applicants, investigators, sponsors, regulators, clinicians, or
decision-makers'' \cite{rfarm27001v2}.

That division is not a courtesy to human readers. It is the reason the system
can be audited at all. A regime in which a model both produced and cleared the
work would have no point at which responsibility attaches, and no funder should
accept one. The regime described here attaches responsibility at exactly one
point, and the whole of \S\ref{sec:peerreview} is an argument that the point is
correctly placed.
