% sec-03-ind.tex - Section 3, IND.  A main section.  FINAL stage.
% Figures 6, 7, 8 and 9 drawn; Tables 8, 9 and 10 populated from the twelve
% section files of trial-ind/final-ind/publication.  Senior-author pass: the
% Module column of Table 8 loses 0.15 cm to its Characters column, which now
% carries six digits and a comma; two closing subsections give the prior
% system's own account of the same work and the four instruments a filing would
% still need.
\section{IND}
\label{sec:ind}
...
% Subject.  The tile is drawn without \dgnodew's default label position, because
% that position is 5.4 mm below the center, which is inside the 7.5 mm halo and
% inside the corridor the four inbound edges converge through.  The label is set
% above the halo instead, on two lines at a 17 mm measure, so it spans
% x = 9.75 to 11.45 and y = -1.55 to -0.95, which no edge reaches.
\node[dgtiled] (ind) at (10.60,-2.55) {};
\glyphdoc{ind}{trialwhite}%
\draw[trialburg,line width=0.6pt] (ind.center) circle (7.5mm);
\node[dglabel,text width=17mm,anchor=south] (indl) at ([yshift=10mm]ind.center)
  {IND v1.0,\\twelve modules};
% composed cluster
...
% Every inbound edge terminates on the halo itself, at a stated bearing, rather
% than on a tile anchor: 145, 180, 215 and 250 degrees, which is a 35 degree
% separation at the arrowhead and no shared terminus.
% The regulatory edge is routed over the top of the canvas rather than straight
% at the subject, because a straight run from cr.east would cross the whole
% simulation cluster and its four tiles.  It leaves at cr's north east, runs at
% y = 2.05 which is 0.50 cm above every cluster title, descends at x = 11.70
% which is 0.25 cm right of the subject label and 1.20 cm left of the composed
% cluster, and enters the halo at 45 degrees.
\draw[dgedgeb] (cr.north east) -- (4.15,2.05) --
  node[above,font=\tiny\sffamily,fill=trialwhite,inner sep=1pt,pos=0.42] {adapted}
  (11.70,2.05) -- (11.70,-2.02) -- (11.13,-2.02);
\draw[dgedgeb] (cd.east) -- node[below,font=\tiny\sffamily,fill=trialwhite,inner sep=1pt] {specified} (9.99,-2.98);
\draw[dgedge] (cs.east) -- (9.85,-2.55);
% The clinical edge crosses the device cluster in the empty band between its two
% tile rows, y = -4.70 to -5.15, rather than through the label row above it.
\draw[dgedge] (cc.east) -- ++(0.45,0) -- (9.30,-5.00) -- (10.34,-3.26);
...
east boundary. Each terminates on the halo at\\its own bearing, and the subject
label sits above the halo, so no inbound edge crosses a label box.}
\end{appfig}
\vspace{-0.6cm}
...
\begin{apptable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.90cm} >{\raggedright\arraybackslash}p{1.55cm} >{\raggedright\arraybackslash}p{2.00cm} >{\raggedright\arraybackslash}X}
...
\foreach \i/\lab/\xa/\xb in {%
  ...
  \fill[trialburgp] (\xa,{-0.35*\i-0.12}) rectangle (\xb,{-0.35*\i+0.12});
  \draw[trialburg,line width=0.3pt] (\xa,{-0.35*\i-0.12}) rectangle (\xb,{-0.35*\i+0.12});}
...
\foreach \i/\lab/\xa/\xb in {%
  ...
  \fill[trialmist] (\xa,{-4.40-0.35*\i-0.12}) rectangle (\xb,{-4.40-0.35*\i+0.12});
  \draw[trialslate,line width=0.3pt] (\xa,{-4.40-0.35*\i-0.12}) rectangle (\xb,{-4.40-0.35*\i+0.12});}
...
deposit, which is the review regime \S\ref{sec:peerreview} describes and measures
...
\subsection{The prior system's own account of the same work}

It is worth being precise about what the prior system does with these twelve
modules, because the comparison in Figure 7 is otherwise an assertion. In the
prior system a Phase 1 IND for a combined drug and device investigation is
produced by a regulatory affairs function working from a protocol supplied by
clinical development, a chemistry section supplied by manufacturing, a
nonclinical section supplied by toxicology, and a brochure assembled from all
three. Each of those inputs arrives on its own schedule; the regulatory function
reconciles them; legal and quality review the reconciliation; and the sponsor
signs. The five stages of Figure 7's lower band are that sequence, and the
months on its axis are the intervals between handoffs rather than the time spent
writing.

That is the structural reason the new system's four days is not merely a faster
version of the same thing. There are no handoffs. The general investigational
plan and the chemistry module were written against one another within the same
day rather than reconciled across two functions in the same quarter, which is
why a number that appears in both appears identically in both without a
reconciliation step. The comprehensive cross-reference discipline reported in
Table 10, 52 codified references across twelve modules, is a consequence of that
adjacency rather than an achievement on top of it.

The corresponding loss is also structural and should be stated. A single author
cannot supply the independent judgment that four functions supply, and no amount
of internal consistency substitutes for it. That is precisely the gap
\S\ref{sec:peerreview} fills with two independent model manufacturers reviewing
during development, and it is the reason the review regime is reported as part
of the method rather than as an afterthought.

\subsection{What the dossier would need to become a filing}

Nothing in \S\ref{sec:ind} is filed. Converting the deposited dossier into an
active IND requires four things the sponsor does not hold: a sponsor-investigator
of record willing to sign Form FDA 1571 for the investigation as described, an
institutional review board of record willing to review a combined IND and IDE
submission carrying a Physical AI consent opt-out, a drug supply agreement for
daraxonrasib, and a device acceptance certificate for the eight-arm platform
described in \S\ref{sec:protocol}. Each is an institutional instrument rather
than a document, and each is named in the register of \S\ref{sec:limitations}.

The dossier is written so that acquiring those four does not require rewriting
it. Every module is separately compilable, every figure is drawn from a
specification rather than from a rendering, and every table is section-scoped
rather than document-scoped, so a change forced by a site's particulars touches
one module and renumbers nothing. That property is worth more to a sponsor than
the four-day production figure, because a filing is amended far more often than
it is written.
That property also sets what a review round costs. A dossier assembled in four
days is answerable to \S\ref{sec:peerreview}'s clock rather than to a journal's:
its twelve modules were read by three model manufacturers against the codified
content items before the deposit, not seven to eight weeks after it, so every
correction landed in the filed document rather than in an erratum. For a disease
with roughly 142 US deaths a day \cite{siegel2025statistics}, the difference
between correcting a dossier before it is filed and correcting it after is a
quarter of a year of enrollment that either happens or does not.
