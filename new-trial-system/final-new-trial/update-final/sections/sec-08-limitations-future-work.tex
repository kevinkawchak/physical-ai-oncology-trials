...
% instruments is expanded so \S\ref{sec:limitations} and \S\ref{sec:funding} agree.
...
checking inside one family detects a shared blind spot. The source study for \S\ref{sec:peerreview}
...
manufacturers of \S\ref{sec:peerreview}, and the residual is the case where all three agree and all
three are wrong.
...
% The divider sits at -3.85, which is 0.21 cm below the first row's cluster
% boundary, and the second row starts at -5.45, which puts its cluster titles
% 0.40 cm below the divider.  At -3.40 and -4.55 the divider ran through both
% second-row titles and both second-row cluster boundaries touched the first
% row's.
\draw[trialslate,dashed,line width=0.4pt] (-0.35,-3.85) -- (14.80,-3.85);
\dgnodeg{dgtilek}{1.05}{-5.45}{\glyphsignal}{Generated text may carry a watermark}{b1}
\dgnodeg{dgtilek}{1.05}{-7.35}{\glyphmon}{Detection policy is not yet settled}{b2}
\dgnodeg{dgtilek}{1.05}{-9.25}{\glyphbank}{Journals differ on disclosure}{b3}
\begin{scope}[on background layer]
...
\dgnode{dgtilem}{8.35}{-5.45}{\glyphdoc}{Provenance disclosed on the cover page}{n1}
\dgnode{dgtilem}{8.35}{-7.35}{\glyphdb}{Commit history is the primary record}{n2}
\dgnode{dgtilem}{8.35}{-9.25}{\glyphserver}{Preprint and repository, not a journal gate}{n3}
\begin{scope}[on background layer]
...
% The residual node is drawn without \dgnodew's default label position, which is
% 5.4 mm below the center and therefore inside the 7.5 mm halo.  The label is set
% 11 mm below the center on a 20 mm measure, so it spans x = 11.95 to 13.95 and
% both inbound edges pass to the left of it.
\node[dgtiled] (res) at (12.95,-5.75) {};
\glyphchart{res}{trialwhite}%
\draw[trialburg,line width=0.6pt] (res.center) circle (7.5mm);
\node[dglabel,text width=20mm,anchor=north] (resl) at ([yshift=-11mm]res.center)
  {Residual: funding decision risk};
...
% Both inbound edges run orthogonally up their own corridor, x = 11.15 and
% x = 11.45, and enter the halo at 135 and 225 degrees.  Neither corridor reaches
% the residual label at x = 11.95, and the two verticals do not overlap because
% one ends at -5.22 and the other begins at -6.28.
\draw[dgedged] (m3.east) -- (10.05,-2.25) -- (11.15,-2.25) -- (11.15,-5.22) -- (12.42,-5.22);
\draw[dgedged] (n3.east) -- (10.05,-9.25) -- (11.45,-9.25) -- (11.45,-6.28) -- (12.42,-6.28);
\pnote{0.35}{-11.05}{Exposures and mitigations share six y values exactly, so every pairing edge is
...
\end{appfig}
\vspace{-0.6cm}
\figcaption{Figure 25. Two structural exposures, single-vendor production and text\\
...
described in \S\ref{sec:ind} and \S\ref{sec:protocol}. The documents exist. The IND is written, both
...
No further document production changes that. Those seven instruments are the
register the whole paper converges on, and none can be produced by writing. The
method of \S\ref{sec:methods} solved the problem it was built for; the problem
that remains is of a different kind.
