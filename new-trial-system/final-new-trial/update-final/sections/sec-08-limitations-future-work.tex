...
No further document production changes that, and the sequence ends here on
purpose. Those seven instruments are the register the whole paper converges on:
four were identified independently in \S\ref{sec:ind} and \S\ref{sec:protocol},
and three are contractual rather than clinical. None can be produced by writing,
which is the sense in which this paper ends a documentary sequence rather than
continuing one. The method of \S\ref{sec:methods} solved the problem it was
built for; the problem that remains is of a different kind.
