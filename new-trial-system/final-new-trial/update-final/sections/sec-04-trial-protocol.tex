...
\begin{apptable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.70cm} >{\raggedright\arraybackslash}p{1.40cm} >{\raggedright\arraybackslash}p{1.80cm} >{\raggedright\arraybackslash}p{2.05cm} >{\raggedright\arraybackslash}p{1.30cm} >{\raggedright\arraybackslash}X}
...
autonomy disclosure duty in \S\ref{sec:legislation} attaches to.
...
equivalents acting alone, which is the point \S\ref{sec:funding} and \S\ref{sec:limitations} return to. The
...
Both protocols carry the review provenance of the dossier of \S\ref{sec:ind}.
The two documents and their shared statistical plan were frozen, hashed and read
by three independent model reviewers, so the guards of Figure 10 and the rungs
of Figure 11 were checked against each other before either was deposited. A
protocol is where a late correction costs most, because a changed eligibility
criterion or dose-limiting toxicity window invalidates the enrollment already
done under it, and \S\ref{sec:peerreview} prices that delay.
