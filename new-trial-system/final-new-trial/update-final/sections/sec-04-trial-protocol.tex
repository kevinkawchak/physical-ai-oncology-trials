...
Both protocols carry the same review provenance as the dossier of
\S\ref{sec:ind}, and for the same reason. The Phase 1 document, the Phase 2
document and their shared statistical plan were frozen, hashed and read by three
independent model reviewers at defined milestones, so the guards of Figure 10
and the rungs of Figure 11 were checked against each other before either
protocol was deposited rather than after. A protocol is the artifact where a
late correction is most expensive, because a changed eligibility criterion or a
changed dose-limiting toxicity window invalidates the enrollment already done
under it, and \S\ref{sec:peerreview} sets out what the prior system's timing
costs when that correction arrives after release.
