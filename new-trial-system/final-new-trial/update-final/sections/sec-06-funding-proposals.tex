...
% FINAL stage.  Figures 17, 18, 19 and 20 drawn; Tables 17 to 20 populated from
...
% Senior-author pass: Table 17's Recipient column loses 0.10 cm to its Mechanism
% column, and two closing subsections state what the money is actually for and
% reduce the two prices to one comparison.
...
\begin{apptable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.80cm} >{\raggedright\arraybackslash}p{4.05cm} >{\raggedright\arraybackslash}p{2.85cm} >{\raggedright\arraybackslash}p{1.95cm} >{\raggedright\arraybackslash}X}
...
\end{apptable}
\vspace{-0.6cm}
\tabcap{Table 17. The ten applications filed in August 2026, the mechanism\\
class each addresses, the term requested, and the ask carried on each.}
...
\foreach \i/\lab/\pa/\pb/\dd in {%
  ...
  \fill[trialburgp] (\pa,{-0.30-0.34*\i-0.11}) rectangle (\pb,{-0.30-0.34*\i+0.11});
  \draw[trialburg,line width=0.3pt] (\pa,{-0.30-0.34*\i-0.11}) rectangle (\pb,{-0.30-0.34*\i+0.11});
  \fill[trialburgl] (\pb,{-0.30-0.34*\i-0.11}) rectangle (\dd,{-0.30-0.34*\i+0.11});
  \draw[trialburg,line width=0.3pt] (\pb,{-0.30-0.34*\i-0.11}) rectangle (\dd,{-0.30-0.34*\i+0.11});}
% today marker
\draw[trialchar,dashed,line width=0.5pt] (12.10,0.30) -- (12.10,-3.40);
% The marker label is set below the foot of its own dashed rule rather than
% above its head, because above it collides with the Aug 29 axis tick at
% x = 12.60: both labels are about 1.5 cm wide and their centers are 0.50 cm
% apart.
\node[font=\tiny\sffamily,anchor=north,text=trialchar] at (12.10,-3.45) {Aug 14, 2026};
...
\end{apptable}
\vspace{-0.6cm}
\tabcap{Table 18. The ten capitalization quantities the plan turns on, and\\
what each one governs in the proposed thirty-three month program.}
...
\end{appfig}
\vspace{-0.6cm}
\figcaption{Figure 18. One four-layer budget priced three ways, and the identical\\
...
% The rotated firewall label is centered at y = -1.80 rather than -3.60, in the
% stretch of the corridor above the first requirement edge.  At -3.60 it ran
% through the "requires" label on the edge at y = -4.16.
\node[gvctitle,rotate=90,anchor=south] at (10.75,-1.80) {firewall, federal money left of the line};
...
\end{apptable}
\vspace{-0.6cm}
\tabcap{Table 19. The twelve sections of the NIH application at version\\
2.0, and what each section had to establish for a reviewer.}
...
property the deposit discipline of \S\ref{sec:methods} exists to produce. The ten applications
...
\subsection{What the money is actually for}

A funder reading Table 18 is entitled to ask what the \$1,606,000 buys that the
existing artifacts do not already supply, given that the IND, both protocols and
the statutory framework are written and deposited. The answer is in Figure 19's
four ports, and none of the four is document production. Port 1 buys the Phase 0
gate evidence: at least 1000 simulated procedures across at least two
independent frameworks, and the maintenance of the verification suite that
produces them. Port 2 buys IND maintenance, amendments and required reports,
which is the recurring regulatory cost of holding an active application rather
than the one-time cost of writing one. Port 3 buys one qualified site and device
acceptance testing. Port 4 buys the first clinical cohorts with monitoring and
adjudication \cite{capplan2026}.

Document production, the thing this paper is about, appears nowhere in the
budget. That is deliberate and it is the strongest statement the funding section
can make: the method of \S\ref{sec:methods} has already removed the cost it was
built to remove, and the remaining \$1,606,000 is the irreducible cost of
running a trial rather than of describing one. A funder who believes the
production-rate claims of \S\ref{sec:ind} through \S\ref{sec:legislation} should
therefore expect the ask to be smaller than a comparable program's, and it is:
\$1,606,000 over thirty-three months against a \$3,500,000 five-year direct
program, with 15.0 percent overhead and fee against a 40 percent allowance not
claimed \cite{capplan2026}.

\subsection{The two prices, read as one sentence}

The arithmetic of Figure 18 reduces to a single comparison a funder can carry
away. The identical four-layer budget of direct work is \$1,396,000 when routed
through a company with no negotiated indirect-cost rate and \$2,137,000 when
routed through a university at a 57 percent facilities and administrative rate
\cite{capplan2026}. The difference, \$741,000, is not a saving on the work; the
work is the same work. It is the price of the route. Nine of the ten
applications in Table 17 address mechanisms that take the second route, and the
capitalization plan and application 05 exist to make the first route available
for the same science.

Above that sits the bridge. Federal money of \$1,606,000 sits below a firewall;
contributed drug supply, operating room time and pathology sit unpriced in the
middle, which is why Figure 19 draws them outside the award and connected to it
by requirement edges rather than by port edges; and \$5,900,000 of private
capital sits above, raised only after a milestone closes, giving 3.67 to one on
cash alone against an annexed target of at least 3 to 1
\cite{capplan2026, kratsiosvought2026fy2028priorities}. A funder is therefore
not being asked to fund a program. It is being asked to fund the gate that
unlocks one, and the twelve costed milestones state in advance what closing that
gate looks like.
The economics of the review round belong in the same ledger. Fourteen funding
artifacts were produced across 74 days at an inference cost measured in tens of
dollars, against list article processing charges of \$2,000 to \$6,000 for a
single paper and above \$10,000 at several flagship titles
\cite{aiprgliomatc}. That asymmetry is not a footnote to the capitalization
argument; it is part of it, because every dollar not spent on placing a
manuscript is a dollar available to the direct program, and every week not spent
waiting on a round is a week of the twelve costed milestones.
\S\ref{sec:peerreview} gives both figures and the ratio between them.
