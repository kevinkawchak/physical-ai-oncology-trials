...
\figslot{plantuml-type / activity with fork and join}{9.4}{Figure 3. The
eight-stage build as one activity: five diagram stages fork and\\join on a shared
specification, then three paper stages run strictly in order.}
...
\figslot{mermaid-type / sequenceDiagram}{8.8}{Figure 4. One generation turn from
master prompt to pushed commit, with the\\four participants, the two return
paths, and the point the author can stop it.}
...
\figslot{graphviz-type / dot record}{7.0}{Figure 5. Two records for one figure:
the machine-readable specification\\and the raster image, with the six fields
only the first returns as input.}
