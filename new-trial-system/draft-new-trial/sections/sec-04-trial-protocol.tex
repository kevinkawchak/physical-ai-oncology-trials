...
Safety Level rating at or above 7.0.
...
\figslot{plantuml-type / state with guards}{9.6}{Figure 10. One participant from
screening to the 24-month endpoint, with the\\guard on every transition and the
two conditions that route to a study hold.}
...
\figslot{mermaid-type / flowchart LR}{7.8}{Figure 11. From the Phase 0 simulation
gate through three 3+3 dose levels to\\Phase 2 randomization, with the quantity
that must hold on each rung shown.}

\figslot{d2-type / containers}{8.4}{Figure 12. The twelve Phase 1 protocol
sections sorted into what Phase 2\\carried unchanged, what it replaced, and what
randomization made necessary.}
...
date, Sections, Figures, Elapsed production, Prior-system analogue. Both rows
...
Both, combined & -- & Jun 21 to 23 & 26 & 4 days against a full trial-design cycle \\
\end{tabularx}
\end{apptable}
\vspace{-0.6cm}
\tabcap{Table 13. Both protocol deposits, their versions and dates, and the\\elapsed
production time against the prior system's design cycle.}

\subsection{The site the protocol assumes}

\figslot{diagrams (python)-type / clustered infrastructure}{9.8}{Figure 13. The
four-layer on-premises site stack, and the single boundary\\across which no
inference request, model weight, or patient record passes.}
