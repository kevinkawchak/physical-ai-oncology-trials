...
eight section files, and an Overleaf zip, and every distinguishable file was a commit pushed in real time. Table 2 summarizes stage outputs.

\vspace{0.2em}
\noindent\begin{tabularx}{\textwidth}{L{2.4cm} L{3.1cm} Y L{2.0cm}}
\toprule
...
with one commit per file set pushed in real time.}
...
for each (Figure 16, Table 3). Node color in Figure 16 encodes the gate type from
Figure 7: maroon for hard gates, steel blue for protocol-defined gates, and
terracotta for decision gates. Two targets are detailed further: the initial IND
...
\node[mmproc] (t3) at (5.0,-3.0) {3. Cohort-review pkg};
...
\node[mmgoal] (out) at (10.4,-3.0) {Compressed\\admin/prep time\\(months to a year)};
\node[mmctx] (note) at (0,-7.6) {Schedule value highest\\on the critical path};
\node[mmdec] (lim) at (10.4,-7.8) {Gains capped by clinical events, fixed review clocks};
\draw[mmedge] (in.east) to[out=60,in=180] (t1.west);
\draw[mmedge] (in.east) to[out=40,in=180] (t2.west);
\draw[mmedge] (in.east) to[out=20,in=180] (t3.west);
\draw[mmedge] (in.east) to[out=-20,in=180] (t4.west);
\draw[mmedge] (in.east) to[out=-40,in=180] (t5.west);
\draw[mmedge] (in.east) to[out=-60,in=180] (t6.west);
\draw[mmedge] (t1.east) to[out=0,in=120] (out.west);
\draw[mmedge] (t2.east) to[out=0,in=150] (out.west);
\draw[mmedge] (t3.east)--(out.west);
\draw[mmedge] (t4.east) to[out=0,in=200] (out.west);
\draw[mmedge] (t5.east) to[out=0,in=225] (out.west);
\draw[mmedge] (t6.east) to[out=0,in=250] (out.west);
\draw[mmedged] (note.north) to[out=90,in=-90] (in.south);
...
common compression of administrative and preparation time; the\\schedule value is
highest on the critical path, and the gains are capped by clinical events and fixed
review clocks.}

\vspace{0.2em}
\noindent\begin{tabularx}{\textwidth}{L{0.4cm} L{4.6cm} L{2.6cm} Y}
\toprule
...
\node[mmaccent] (acc) at (2.8,-6.2) {Faster assembly\\starts clock + sites sooner};
\node[mmdec] (lim) at (7.2,-6.75) {Cannot shorten\\30 days or make\\missing data};
\draw[mmedge] (c1) to[out=0,in=180, looseness=0.4] (pkg.west);
\draw[mmedge] (c2) to[out=0,in=180, looseness=0.4] (clk.west);
\draw[mmedge] (c3) to[out=0,in=200, looseness=0.4] (clk.west);
\draw[mmedge] (c4) to[out=180,in=0, looseness=0.4] (pkg.east);
\draw[mmedge] (c5) to[out=180,in=0, looseness=0.4] (clk.east);
\draw[mmedge] (c6) to[out=180,in=-20, looseness=0.4] (clk.east);
\draw[mmedge] (pkg)--(clk);
\draw[mmedge] (clk.south) to[out=-120,in=90, looseness=0.4] (acc.north);
\draw[mmedge] (clk.south) to[out=-60,in=90, looseness=0.4] (lim.north);
\end{mermaidfig} 
\vspace{-0.7cm}
...
\node[mmproc] (rev) at (8.4,-1.9) {FDA review:\\30 calendar days};
\node[mmgoal] (g) at (8.4,-3.8) {Hold lifted;\\enrollment resumes};
\node[mmaccent] (acc) at (4.4,-3.0) {Faster assembly\\moves restart forward};
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 18. The 30-day FDA review
...
\begin{mermaidfig}
\node[mmdec] (b1) at (0.1,0) {Clinical / operational\\NOT compressible};
\node[mmaccent] (b2) at (5.0,0) {Administrative / prep\\COMPRESSIBLE};
\node[mmdec] (b3) at (9,0) {Regulatory review\\FIXED};
\node[mmctx] (b1d) at (0.1,-3.3) {Recruitment, treatment,\\PFS/OS maturation};
\node[mmin] (b2d) at (5.0,-1.7) {Data cleaning, analysis,\\document writing};
\node[mmctx] (b3d) at (9,-3.3) {30-day IND review,\\30-day hold review};
\node[mmproc] (llm) at (5.0,-3.5) {Repository LLM\\(acts on this bucket only)};
\node[mmgoal] (g) at (5.0,-5.2) {months to a year\\saved cumulatively};
...
preparation bucket compresses; clinical\\operational bucket and fixed
regulatory review clocks do not. Repository LLM acts on middle bucket only.}
...
matching. Quality gates that prevent a document from generating a rework cycle
(amendments, regulator questions, IRB revisions, or extra review cycle) are in Figure 21.

\begin{mermaidfig}
\node[mmgoal] (g) at (9.4,-4.25) {Grounded,\\monitorable paper};
\node[mmdec] (k1) at (0.2,-0.25) {1 Grounding};
\node[mmdec] (k2) at (0.2,-2.2) {2 Figures match context};
\node[mmdec] (k3) at (0.2,-4.3) {3 GitHub real-time};
\node[mmdec] (k4) at (0.2,-6.4) {4 Repo/dir names match};
\node[mmdec] (k5) at (0.2,-8.4) {5 File names match};
\node[mmin] (e1) at (4.7,-0.25) {24 Mermaid figures};
\node[mmin] (e2) at (4.7,-2.2) {cited where discussed};
\node[mmin] (e3) at (4.7,-4.3) {one commit per file};
\node[mmin] (e4) at (4.7,-6.4) {trial-documents/...};
\node[mmin] (e5) at (4.7,-8.4) {sec-04-results.tex};
\draw[mmedge] (k1)--(e1); \draw[mmedge] (k2)--(e2); \draw[mmedge] (k3)--(e3);
\draw[mmedge] (k4)--(e4); \draw[mmedge] (k5)--(e5);
\draw[mmedge] (e1) to[out=0,in=150] (g.west);
\draw[mmedge] (e2) to[out=0,in=170] (g.west);
\draw[mmedge] (e3)--(g.west);
\draw[mmedge] (e4) to[out=0,in=190] (g.west);
\draw[mmedge] (e5) to[out=0,in=210] (g.west);
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 20. Five name-matching and grounding verifications, each with a concrete example.}
...
\begin{mermaidfig}
\node[mmin] (d) at (0,-2.2) {Draft document};
\node[mmdec] (q1) at (0,0) {consistent?};
\node[mmdec] (q2) at (4,0) {validated data?};
\node[mmdec] (q3) at (8.5,0) {DOI/URL,\\single dashes?};
\node[mmgoal] (g) at (8.5,-2.2) {Submission-ready\\(no extra cycle)};
\node[mmaccent] (rev) at (4,-2.2) {Revise};
\node[mmctx] (note) at (4,-4.4) {poor docs add amendments,\\regulator questions,\\IRB revisions};
\draw[mmedge] (d.north)--(q1); \draw[mmedge] (q1)--(q2); \draw[mmedge] (q2)--(q3);
\draw[mmedge] (q3)--(g);
\draw[mmedged] (q1) to (rev.north west);
\draw[mmedged] (q2) to[out=-90,in=90] (rev.north);
\draw[mmedged] (rev) to[out=180,in=-0] (d.east);
\draw[mmedged] (rev)--(note);
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 21. A draft that fails consistency, data
support, or formatting checks is revised rather than submitted, preventing
...
\vspace{0.2em}
\noindent\begin{tabularx}{\textwidth}{L{6.4cm} L{3.6cm} Y}
\toprule
...
clinical decision support \cite{bevacds, costmab, autollmagent} through glioblastoma
meta-analysis
...
\node[mmin] (y2) at (3.75,0) {2025 H1\\cost, CDS, GBM meta, lung};
\node[mmproc] (y3) at (7.5,0) {2025 H2\\PDAC twin, 100k Phase III};
\node[mmproc] (y4) at (11.2,0) {Dec 2025\\16-LLM code competition};
...
\node[mmaccent] (this) at (9.4,-4.2) {This paper (v1.0)\\single-prompt document generation};
\draw[mmedge] (y1)--(y2); 
\draw[mmedge] (y2)--(y3); 
\draw[mmedge] (y3)--(y4);
% \draw[mmedge] (y4.south east) to (y5.north);
\draw[mmedge] (y6)--(y7); 
\draw[mmedge] (y5)--(y6);
% \draw[mmedge] (y5.south) to (this.west);
\draw[mmedge] (y7.south) to (this.north);
\end{mermaidfig}
\vspace{-0.65cm}
...
\vspace{0.2em}
\noindent\begin{tabularx}{\textwidth}{L{1.8cm} L{6cm} Y}
\toprule
...
2025-08 & QSP metastatic PDAC simulation \cite{kawchak_2025_17001137} & Code, VVUQ, playbook: protocol to predict \\
