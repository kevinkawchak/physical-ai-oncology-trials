...
\begin{mermaidfig}
\node[mmgoal] (mp) at (0,0) {Master prompt\\prompts/prompt-paper.md};
...
\node[mmproc] (rel) at (0,-11.7) {README + releases + CHANGELOG v4.2.0\\Edits, Publication};
\node[mmctx] (gh) at (6.0,-5.85) {GitHub\\real-time\\auto-commit\\auto-PR monitor};
\draw[mmedge] (mp)--(a);
\draw[mmedge] (a)--(s1);
\draw[mmedge] (s1)--(s2);
\draw[mmedge] (s2)--(s3);
...
\draw[mmedged] (s4.east) to[out=0,in=240] (gh.south);
\draw[mmedged] (gh.north) to[out=120,in=20] (s1.east);
\end{mermaidfig}
\vspace{-0.7cm}
...
auto-commits and opens a pull request on GitHub in real time, and\\the curved
feedback edge shows author review returning into the build, all without manual
intervention.}

\begin{mermaidfig}
\node[mmdark] (t0) at (0,0) {Traditional baseline\\weeks to months\\per iteration};
...
\node[mmin] (n1) at (7.2,-1.8) {draft scaffold};
\node[mmin] (n2) at (7.2,-3.3) {full version};
\node[mmin] (n3) at (7.2,-4.8) {final polish};
\node[mmgoal] (g) at (3.6,-6.4) {Administrative / prep\\time compressed};
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 2. The traditional
...
human-in-the-loop review of commits, Mermaid grounding, paper-to-repository
...
\begin{mermaidfig}
\node[mmin] (r1) at (0.35,0) {Possible LLM error\\(server/massive input)};
\node[mmin] (r2) at (0.35,-1.7) {Formatting artifacts};
\node[mmin] (r3) at (0.35,-3.4) {Hallucinated context};
...
\node[mmgoal] (g) at (9.2,-4.2) {Benefit $>$ risk for\\terminal patients};
...
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 3. Each probable risk
...
Figure and Table guidance it turns into a worked Phase 1 pancreatic cancer document-generation
study.

The contributions are fourfold: (1) a single-prompt mermaid to draft to full
