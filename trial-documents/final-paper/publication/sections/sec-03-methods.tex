...
input is kept under 5 MB (0.84 MB prior to publication files), and individual README files are kept under roughly 25K
...
downstream reuse. Figure 4 shows architecture.

\begin{mermaidfig}
\node[mmin] (i1) at (0,0) {inputs/llm-adoption\\(paper template)};
\node[mmin] (i2) at (0,-1.7) {inputs/references.bib\\(author works)};
\node[mmin] (i3) at (0,-3.4) {research/*, chunked\\files, READMEs};
\node[mmproc] (ctx) at (4.5,-1.7) {Total input $<$ 5 MB\\README $<$ 25K tokens};
\node[mmproc] (gen) at (9.0,-1.7) {sub-prompts produce\\sectioned outputs};
\node[mmgoal] (o1) at (0,-5) {mermaid};
\node[mmin] (o2) at (4,-5) {draft-paper};
\node[mmaccent] (o3) at (8,-5) {full-paper};
\node[mmgoal] (o4) at (12,-5) {final-paper};
\draw[mmedge] (i1) to[out=0,in=120] (ctx.west);
\draw[mmedge] (i2)--(ctx.west);
\draw[mmedge] (i3) to[out=0,in=240] (ctx.west);
\draw[mmedge] (ctx)--(gen);
\draw[mmedge] (gen.south) to[out=-90,in=70, looseness=0.6] (o2.north);
\draw[mmedge] (gen.south) to[out=-90,in=70] (o3.north);
\draw[mmedge] (o1)--(o2); \draw[mmedge] (o2)--(o3); \draw[mmedge] (o3)--(o4);
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 4. Curated inputs feed a 1M-context model under two size limits; sub-prompts produce
...
\begin{mermaidfig}
\node[mmgoal] (mp) at (5.4,-0.2) {Master prompt\\generate then execute};
\node[mmin] (sp1) at (0,-2.0) {prompt-1-mermaid};
\node[mmin] (sp2) at (3.6,-2.0) {prompt-2-draft};
\node[mmin] (sp3) at (7.2,-2.0) {prompt-3-full};
\node[mmin] (sp4) at (10.8,-2.0) {prompt-4-final};
\node[mmproc] (st1) at (0,-4) {Stage 1\\mermaid};
\node[mmproc] (st2) at (3.6,-4) {Stage 2\\draft};
\node[mmproc] (st3) at (7.2,-4) {Stage 3\\full};
\node[mmgoal] (st4) at (10.8,-4) {Stage 4\\final};
\node[mmaccent] (acc) at (5.4,-6) {context\\across stages};
\draw[mmedge] (mp) to[out=180,in=90, looseness=0.7] (sp1.north);
\draw[mmedge] (mp)--(sp2); \draw[mmedge] (mp)--(sp3);
\draw[mmedge] (mp) to[out=0,in=90, looseness=0.7] (sp4.north);
...
\draw[mmedged] (st3.south) to[out=5,in=85] (acc.north);
\draw[mmedged] (acc.east) to[out=0,in=-100] (st4.south);
\end{mermaidfig}
\vspace{-0.7cm}
...
\node[mmproc] (d1) at (4.4,0) {SAE/SUSAR 7/15-day};
\node[mmproc] (d2) at (9,1.1) {DSUR (annual)};
\node[mmproc] (d3) at (9,-0.4) {Protocol amendments};
\node[mmproc] (d4) at (9,-1.9) {Dose-escalation minutes};
\node[mmgoal] (a1) at (9,-3.6) {TLFs};
\node[mmgoal] (a2) at (9,-5.1) {CSR (ICH E3)};
\node[mmgoal] (a3) at (4.4,-4.5) {Manuscripts / lay summaries};
\draw[mmedge] (b1.east) to (hub.west);
\draw[mmedge] (b2.east) to (hub.west);
\draw[mmedge] (b3.east) to (hub.west);
\draw[mmedge] (b4.east) to (hub.west);
\draw[mmedge] (hub.north) to (d1.south);
\draw[mmedge] (hub.east) to (d2.west);
\draw[mmedge] (hub.east) to (d3.west); 
\draw[mmedge] (hub.east) to (d4.west);
\draw[mmedge] (hub.east) to (a1.west);
\draw[mmedge] (hub.east) to (a2.west);
\draw[mmedge] (hub.south) to (a3.north);
\end{mermaidfig}
\vspace{-0.7cm}
...
shows taxonomy and Table 1 states it in full.

\begin{mermaidfig}
\node[mmdec] (q) at (4.6,-0.3) {Is the document\\on the critical path?};
\node[mmgoal] (g1) at (0,-2.6) {Hard gate};
\node[mmproc] (g2) at (4.6,-2.6) {Protocol-defined gate};
\node[mmaccent] (g3) at (9.2,-2.6) {Decision gate};
\node[mmin] (e1) at (0,-4.575) {IND, IRB approval,\\clinical-hold response,\\material amendments};
\node[mmin] (e2) at (4.6,-4.4) {cohort-review pkg,\\interim SAP / DSMB};
\node[mmin] (e3) at (9.2,-4.4) {EOP2 briefing,\\go/no-go at lock};
\node[mmctx] (n) at (4.6,-6.3) {Faster authoring helps only on the critical path};
\draw[mmedge] (q) to (g1.north);
\draw[mmedge] (q)--(g2);
\draw[mmedge] (q) to (g3.north);
...
\vspace{0.2em}
\noindent\begin{tabularx}{\textwidth}{L{2.8cm} L{4.4cm} Y}
\toprule
...
\node[mmproc] (d1) at (4.2,0) {eCRF: dose, PK/PD};
\node[mmproc] (d2) at (4.2,-1.7) {eCOA / ePRO symptoms};
\node[mmproc] (d3) at (4.2,-3.4) {AE logs (CTCAE),\\DLT in 3+3 window};
\node[mmproc] (a1) at (8.6,-0.85) {SDV by CRAs};
\node[mmgoal] (a2) at (8.6,-2.55) {Database LOCK};
...
\draw[mmedge] (a1)--(a2);
\draw[mmedge] (a2) to[out=0,in=180] (a3.west);
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 8. The before / during / after data-collection pipeline, from
EHR extraction\\through EDC, eCRF, eCOA/ePRO, and CTCAE-graded adverse events to
...
\begin{mermaidfig}
\node[mmin] (t1) at (0.3,0) {SAE / SUSAR event};
\node[mmin] (t2) at (0.3,-1.6) {Emerging PK/PD};
\node[mmin] (t3) at (0.3,-3.2) {Cohort completion};
...
\node[mmproc] (tlf) at (3.8,-1.5) {Biostatistics:\\TLFs};
\node[mmproc] (csr) at (7.6,-1.5) {Writers: narrative\\-> CSR (ICH E3)};
\node[mmaccent] (pi) at (11.4,-1.5) {PI review,\\sign-off};
\node[mmin] (m1) at (7.6,-0.2) {Manuscripts / abstracts};
\node[mmin] (m2) at (7.6,-2.8) {Lay summaries};
\draw[mmedge] (lk)--(tlf); \draw[mmedge] (tlf)--(csr); \draw[mmedge] (csr)--(pi);
\draw[mmedge] (csr.north) to (m1.south);
\draw[mmedge] (csr.south) to (m2.north);
\end{mermaidfig}
\vspace{-0.7cm}
...
\texttt{trial-protocol/final-protocol/publication}. Paper section architecture
maps each section to a \texttt{.tex} file (Figure 14), and figure grounding with Mermaid sources (Figure 15).

\clearpage

\begin{mermaidfig}
\node[mmgoal] (c1) at (4,0) {goal \#8B2E3F};
\node[mmproc] (c2) at (4,-1.5) {proc \#2F5D7C};
\node[mmaccent] (c3) at (4,-3.0) {accent \#D08770};
\node[mmin] (c4) at (11,1) {input \#BFD7EA};
\node[mmctx] (c5) at (7.4,1) {ctx \#F4F7F9};
\node[mmdec] (c6) at (8.8,-3.5) {warn \#D9D9D9};
\node[mmproc] (t) at (8.8,-1.5) {TikZ mm*\\node style};
\node[mmgoal] (g) at (12.7,-1.5) {LaTeX specific\\manuscript figure};
\draw[mmedge] (c1) to[out=0,in=120, looseness=0.7] (t.north west);
\draw[mmedge] (c2)--(t); 
\draw[mmedge] (c3) to[out=0,in=210, looseness=0.7] (t.south west);
\draw[mmedge] (c4.south) to (t.north);
\draw[mmedge] (c5)--(t.north); 
\draw[mmedge] (c6.north) to (t.south);
\draw[mmedge] (t)--(g);
\end{mermaidfig}
\vspace{-0.7cm}
...
\node[mmproc] (g3) at (7.2,0) {push to branch};
\node[mmaccent] (g4) at (10.8,0) {single PR\\(continuously updated)};
\node[mmgoal] (g5) at (10.8,-2.2) {author reviews\\on GitHub};
...
\draw[mmedged] (g6.west) to[out=180,in=-90] (g1.south);
\end{mermaidfig}
\vspace{-0.7cm}
...
\begin{mermaidfig}
\node[mmproc] (main) at (0,-3.75) {main.tex\\\textbackslash input assembles all};
\node[mmgoal] (s1) at (-4.5,-1.5) {Abstract + Keywords};
\node[mmproc] (s2) at (-4.5,-3.0) {Introduction};
\node[mmctx] (s3) at (-4.5,-4.5) {Table of Contents};
\node[mmproc] (s4) at (-4.5,-6.0) {Methods};
\node[mmproc] (s5) at (4.5,-1.5) {Results};
\node[mmproc] (s6) at (4.5,-3.0) {Discussion};
\node[mmproc] (s7) at (4.5,-4.5) {Limitations};
\node[mmgoal] (s8) at (4.5,-6.0) {Conclusions};
\node[mmctx] (s9) at (0,-7) {References + Back Matter (CC)};
\draw[mmedge] (main.west) to[out=180,in=0] (s1.east);
\draw[mmedge] (main.west) to[out=180,in=0] (s2.east);
\draw[mmedge] (main.west) to[out=180,in=0] (s3.east);
\draw[mmedge] (main.west) to[out=180,in=0] (s4.east);
\draw[mmedge] (main.east) to[out=0,in=180] (s5.west);
\draw[mmedge] (main.east) to[out=0,in=180] (s6.west);
\draw[mmedge] (main.east) to[out=0,in=180] (s7.west);
\draw[mmedge] (main.east) to[out=0,in=180] (s8.west);
\draw[mmedge] (main.270) to[out=270,in=90] (s9.north);
\end{mermaidfig}
\vspace{-0.7cm}
...
\node[mmproc] (d) at (4.1,0) {nodes, edges, palette,\\quantitative data};
\node[mmproc] (t) at (8.2,0) {TikZ mermaidfig\\(mm* styles)};
\node[mmgoal] (l) at (12.3,0) {Identical figure\\in draft/full/final};
\node[mmdec] (v) at (6.15,-2.2) {Verify twice:\\no overlaps, looseness,\\box spacing};
\draw[mmedge] (m)--(d); \draw[mmedge] (d)--(t); \draw[mmedge] (t)--(l);
\draw[mmedged] (v.north west) to (d.south);
\draw[mmedged] (v.north east) to (t.south);
\end{mermaidfig}
\vspace{-0.7cm}
\figcaption{Figure 15. Figure grounding: each Mermaid source is reproduced as a TikZ
\texttt{mermaidfig} of\\the same complexity, then verified twice for overlaps, arrow
