...
traceable to a source and every figure traceable to a Mermaid file? The quality
gates of Figure 21 are the operational answer.
...
\begin{mermaidfig}
\node[mmin] (pt) at (0,0) {KRAS-mutated PDAC\\daraxonrasib + robotic Whipple};
\node[mmproc] (p1) at (4.7,0) {Phase 1 IND/IDE protocol};
\node[mmproc] (p2) at (9.2,0) {3+3 cohort review\\establish RP2D};
\node[mmproc] (p3) at (9.2,-2.0) {Amendments +\\synchronized consent};
\node[mmproc] (p4) at (4.7,-2.0) {Phase 1-to-2 CSR\\and EOP2 briefing};
\node[mmgoal] (p5) at (0,-2.0) {Phase 2 randomized\\protocol};
\node[mmaccent] (acc) at (4.7,-3.8) {each document built by the single-prompt workflow};
\draw[mmedge] (pt)--(p1); \draw[mmedge] (p1)--(p2);
\draw[mmedge] (p2)--(p3); \draw[mmedge] (p3)--(p4); \draw[mmedge] (p4)--(p5);
\draw[mmedged] (acc) to[out=120,in=-90] (p4.south);
\end{mermaidfig}
...
and transition documents to the Phase 2 randomized protocol, each built by the
single-prompt workflow.}

\begin{mermaidfig}
\node[mmaccent] (a) at (0,0) {Faster validated\\authoring};
\node[mmproc] (b) at (4.4,0) {Earlier IND/IRB,\\site activation};
\node[mmproc] (c) at (8.8,0) {Earlier first dose,\\faster cohorts};
...
\node[mmctx] (g) at (4.4,-3.8) {does not change tumor biology or fixed review clocks};
\draw[mmedge] (a)--(b); \draw[mmedge] (b)--(c); \draw[mmedge] (c)--(d);
\draw[mmedge] (d)--(e); \draw[mmedge] (e)--(f);
\draw[mmedged] (g) to[out=120,in=-90] (f.south);
\end{mermaidfig}
...
and, for this population, extending progression-free and overall survival, without
