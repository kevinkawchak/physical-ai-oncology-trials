...
For Physical AI trials, the investigator interface includes additional capabilities specific
to robotic procedure oversight. Investigators can review pre-operative digital twin
simulations before authorizing Tier 3 procedures, access real-time robotic telemetry during
procedures (if the procedure protocol permits investigator observation), review post-procedure
telemetry summaries, and approve procedure provenance records for inclusion in the trial
database. These capabilities extend the traditional investigator role from oversight of drug
administration to oversight of both drug and robotic procedure administration.
...
Digital health technology integration configures and manages the wearable devices, mobile
applications, and remote monitoring systems used in decentralized trial elements. The
patient interface agent validates device compatibility, deploys device configuration
profiles, monitors data transmission reliability, and manages device replacement when
failures occur. For Physical AI trials, DHT integration extends to post-procedure
monitoring devices that track recovery metrics following robotic surgical procedures,
enabling remote assessment of surgical outcomes between scheduled follow-up visits.

Patient education content delivery provides participants with accessible information
about their trial participation, including the robotic procedures they will undergo. The
content is tailored to the patient's treatment plan: a patient scheduled for da Vinci Xi
robotic surgery receives education about the surgical robot, the pre-operative and
post-operative process, and the safety monitoring systems; a patient receiving
Franka Emika-prepared chemotherapy receives education about the dose preparation process
and the verification steps that ensure accuracy. This education content addresses the
unique informed consent requirements for Physical AI procedures specified in the adapted
21 CFR Part 50 \cite{cfr50-adapt}.

The patient interface also manages the patient portal, which provides participants with
a unified view of their trial participation: upcoming appointments, completed visits,
medication schedules, ePRO questionnaire deadlines, and contact information for the
research team. The portal integrates with the site scheduling system and the robot
execution gateway to display real-time appointment status, including any schedule
changes due to robotic system maintenance or scheduling conflicts.
