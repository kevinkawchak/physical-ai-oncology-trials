"""Sponsor activity script for Hour 02 (Early morning pre-dawn operations with gradual ramp-up)."""
...
HOUR = 2
NEW_TRIAL_DIR = Path(__file__).resolve().parents[4] / "new-trial" / "hour-02"
PATIENT_COUNT = 3
...
def _load_trial_data() -> dict:
    ...
    patient_file = NEW_TRIAL_DIR / "hour_02_patient_records.md"
    robot_file = NEW_TRIAL_DIR / "hour_02_robot_logs.md"
    sim_file = NEW_TRIAL_DIR / "hour_02_simulation.md"
...
def generate_sponsor_directives(hour: int = HOUR) -> dict:
    """Generate 12 sponsor decisions at 5-min intervals for hour 02.

    Parameters
    ----------
    hour : int
        The hour (0-23) to generate directives for. Defaults to 2.
    ...
    decisions: list[dict] = [
        {
            ...
            "safety_gate": "G1",
            "rationale": "Initialize hour-02 monitoring cycle for 3 patients",
        },
        {
        ...
        {
            "timestamp": "2026-03-23T02:25:00Z",
            ...
            "escalation_required": False,
            "safety_gate": "G2",
            "rationale": "Confirm consumable levels for hour-02 procedures",
        },
        {
            ...
            "escalation_required": False,
            "safety_gate": "G3",
            "rationale": "Validate CRF data integrity for hour 02",
        },
        {
            "timestamp": "2026-03-23T02:35:00Z",
        ...
        },
        {
            ...
            "escalation_required": False,
            "safety_gate": "G3",
            "rationale": "Discharge readiness for hour-02 treatment cycle",
        },
        {
            "timestamp": "2026-03-23T02:55:00Z",
