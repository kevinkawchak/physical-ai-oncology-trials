...
class VendorManagerAgent:
    ...
    def create_contract(self, vendor_name: str, category: VendorCategory, value_usd: float) -> VendorContract:
    ...
    def add_sla_metric(self, contract_id: str, name: str, target: float, unit: str = "") -> SLAMetric:
        metric = SLAMetric(
            metric_id=f"SLA-{len(self.contracts[contract_id].sla_metrics) + 1:02d}", name=name, target=target, unit=unit
        )
    ...
    def vendor_sla_dashboard(self, contract_id: str) -> list[dict[str, object]]:
        ...
        return [
            {"metric": m.name, "target": m.target, "actual": m.actual, "status": m.status.value}
            for m in contract.sla_metrics
        ]

    def conduct_review(
        self, vendor_name: str, period: str, quality: float, timeliness: float, communication: float
    ) -> PerformanceReview:
    ...
    def summary(self) -> dict[str, object]:
        ...
        breached = sum(1 for c in self.contracts.values() for m in c.sla_metrics if m.status == SLAStatus.BREACHED)
        return {
