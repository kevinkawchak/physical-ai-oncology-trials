...
class TimelineCompressionAnalyzer:
    ...
    def total_comparison(self) -> dict[str, object]:
        ...
        return {"traditional_days": trad, "autonomous_days": auto,
                "days_saved": trad - auto, "compression_pct": round((1 - auto / trad) * 100, 1) if trad else 0.0}

    def delay_cost_analysis(self, daily_revenue: float = 1_500_000,
                            daily_ops_cost: float = 85_000) -> DelayCostAnalysis:
        comp = self.total_comparison()
        days = comp["days_saved"]
        rev = daily_revenue * days
        ops = daily_ops_cost * days
        return DelayCostAnalysis(daily_revenue_at_risk=daily_revenue, daily_operational_cost=daily_ops_cost,
                                 days_saved=days, revenue_recovered=rev, cost_avoided=ops,
                                 total_value=rev + ops)
    ...
    def sensitivity_analysis(self, base_daily_revenue: float = 1_500_000) -> list[SensitivityResult]:
        ...
        for name, base, low_mult, high_mult in parameters:
            ...
            results.append(SensitivityResult(parameter=name, base_value=base,
                                             low_value=base * low_mult, high_value=base * high_mult,
                                             base_savings=round(base_sav, 2),
                                             low_savings=round(low_sav, 2), high_savings=round(high_sav, 2)))
        return results

    def phase_detail(self) -> list[dict[str, object]]:
        return [{"phase": p.phase.value, "traditional": p.traditional_days,
                 "autonomous": p.autonomous_days, "saved": p.compression_days,
                 "compression_pct": round(p.compression_pct, 1)} for p in self.phases]

    def summary(self) -> dict[str, object]:
        return {"study_id": self.study_id, "comparison": self.total_comparison(),
