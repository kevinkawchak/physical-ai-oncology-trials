...
The tables, listings, and figures (TLF) automation subsystem generates the statistical
outputs that accompany the CSR and regulatory submissions. Tables include demographic
summaries, efficacy endpoint analyses, adverse event summaries, laboratory shift tables,
and concomitant medication summaries. Listings include subject disposition listings,
protocol deviation listings, adverse event listings, and individual patient data listings.
Figures include Kaplan-Meier survival curves, waterfall plots, spider plots, and forest
plots. The TLF subsystem interfaces with the biostatistics agent's analysis outputs and
formats them according to the sponsor's standard TLF templates, ensuring consistency
across studies and regulatory submissions.
...
The archiving system implements role-based access controls that distinguish between
archive administrators (who manage the archive infrastructure), archive users (who
retrieve records for regulatory inspection or legal proceedings), and archive auditors
(who verify archive integrity and completeness). The EU CTR requirement for
sponsor-appointed archive persons is satisfied through the designation of specific
human individuals who hold archive administrator credentials and bear accountability
for archive integrity throughout the 25-year retention period.

The transition from active trial to archived trial follows a defined closeout procedure.
Upon completion of all regulatory submissions and disclosure obligations, the quality
agent initiates the archival process: active system data is migrated to the archive
(with integrity verification at each step), active system access is restricted to
read-only mode, the archived data is verified against the active system to confirm
completeness, and the active system is decommissioned after successful archive
verification. The entire closeout process generates audit trail documentation that
becomes part of the archived record.
